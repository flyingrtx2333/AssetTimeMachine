...
| 风险贡献再分配 | 实验 | 10.59% | 9.49% | 9.15% | 9.49% | 1.16 |
| 双引擎制度路由 | 实验 | 11.60% | 9.49% | 10.39% | 9.49% | 1.18 |
| 双引擎水下恢复 | 实验 | 12.10% | 9.44% | 10.91% | 9.44% | 1.19 |
| 双趋势金纳杠铃 | 独立 | 10.15% | 16.98% | 13.56% | 16.98% | 0.89 |

前三条精选策略共享黄金/权益元策略底座，分别代表进取、均衡和稳健风险档位。`双趋势金纳杠铃` 是独立双资产路线。四条实验策略分别承担不同目标：`凸性极速空头组合` 在三条底层逻辑的目标权重上加入3%的严格T−1指数空头模型，总风险最高110%，负现金按5%年化融资；该空头袖套是研究用反向收益模型，不代表可直接购买的单一产品。`风险贡献再分配` 先按40%高夏普状态机、25%进取风险预算和35%双趋势金纳统一融合，并按底层仓位共识使用1.00/1.20/1.40倍风险档；每42个交易日估计最多126日协方差，当单一资产风险贡献超过65%时，将60%目标权重向低波动、低正相关资产重新分配。它包含两层严格T−1保护：其一，美股既有仓位至少10%、目标拟一次增加10%至20%，且纳指与标普近5日动量同时转负时，只执行新增仓位的60%；其二，A股目标一次减少至少15%、美股目标拟增加5%至10%，且纳指或标普近5日动量至少一个未转正时，只执行美股新增仓位的50%，避免区域风险退出后立即把风险转移到美股。正常减仓、普通增仓和大级别状态切换不受影响。当前固定基线中，两层保护分别影响2010/2012与2015年的少数调仓，最近十年、2020年后和2022年后指标均保持不变。目标变化超过8%才成交。它替换了较弱的共识变速版本，但仍未达到8%最大回撤目标，也不会替换默认的“均衡权益状态”。`双引擎制度路由` 的固定基线为11.60%年化收益、9.49%最大回撤和1.18夏普。`双引擎水下恢复` 在其上加入快速上涨桥接及15%长水下恢复袖套，固定基线为12.10%年化收益、9.44%最大回撤和1.19夏普；两者均已加入 App 产品策略目录，但不替换默认策略。所有数字均已扣除1%交易费与0.05%滑点，夏普继续按日收益、零无风险利率计算。
