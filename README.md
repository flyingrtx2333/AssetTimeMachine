...
前三条精选策略共享黄金/权益元策略底座，分别代表进取、均衡和稳健风险档位。`双趋势金纳杠铃` 是独立双资产路线。两条实验策略分别承担不同目标：`凸性极速空头组合` 在三条底层逻辑的目标权重上加入3%的严格T−1指数空头模型，总风险最高110%，负现金按5%年化融资；该空头袖套是研究用反向收益模型，不代表可直接购买的单一产品。`风险贡献再分配` 先按40%高夏普状态机、25%进取风险预算和35%双趋势金纳统一融合，并按底层仓位共识使用1.00/1.20/1.40倍风险档；每42个交易日估计最多126日协方差，当单一资产风险贡献超过65%时，将60%目标权重向低波动、低正相关资产重新分配。它包含两层严格T−1保护：其一，美股既有仓位至少10%、目标拟一次增加10%至20%，且纳指与标普近5日动量同时转负时，只执行新增仓位的60%；其二，A股目标一次减少至少15%、美股目标拟增加5%至10%，且纳指或标普近5日动量至少一个未转正时，只执行美股新增仓位的50%，避免区域风险退出后立即把风险转移到美股。正常减仓、普通增仓和大级别状态切换不受影响。当前固定基线中，两层保护分别影响2010/2012与2015年的少数调仓，最近十年、2020年后和2022年后指标均保持不变。目标变化超过8%才成交。它替换了较弱的共识变速版本，但仍未达到8%最大回撤目标，也不会替换默认的“均衡权益状态”。所有数字均已扣除1%交易费与0.05%滑点，夏普继续按日收益、零无风险利率计算。
