#!/usr/bin/env python3
"""Run preregistered ATM-SVP2-TIC-001 through the shared Swift engine."""
...
FRAGMENT = ROOT / "tools/tic_foreign_equity_flow_v1.swiftpart"
LOGIC = ROOT / "tools/tic_foreign_equity_flow_v1_logic.swift"
ASSEMBLED = Path("/private/tmp/atm_tic_foreign_equity_flow_v1.swift")
BINARY = Path("/private/tmp/atm_tic_foreign_equity_flow_v1")
...
def validate_smoke(stdout: str) -> dict:
    ...
    ids = re.findall(r"^SMOKE_TIC_PATH=(.+)$", stdout, flags=re.MULTILINE)
    source_counts = [int(value) for value in re.findall(r"^SMOKE_TIC_SOURCE_COUNT=(\d+)$", stdout, flags=re.MULTILINE)]
    derisk_counts = [int(value) for value in re.findall(r"^SMOKE_TIC_US_DERISK_COUNT=(\d+)$", stdout, flags=re.MULTILINE)]
    available_counts = [int(value) for value in re.findall(r"^SMOKE_TIC_AVAILABLE_COUNT=(\d+)$", stdout, flags=re.MULTILINE)]
    ...
    return {
        "source_fingerprint": fingerprint.group(1), "identity_match": True, "ids": ids,
        "source_events": source_counts[0], "us_derisk_events": derisk_counts[0],
        "available_us_derisk_events": available_counts[0], "factor_points": int(factor_count.group(1)),
    }
...
def write_outputs(output_dir: Path, rows: dict[str, dict]) -> dict:
    ...
    document = {
        ...
        "strategy_lineage": "tic-foreign-equity-flow-v1",
        "v11_control": v11, "matched_control": matched,
    ...
    with (output_dir / "candidate-metrics.csv").open("w", encoding="utf-8", newline="") as handle:
        ...
        for path_id, kind in [("V11-CONTROL", "CONTROL"), (MATCHED_CONTROL_ID, "MATCHED_CONTROL")]:
            path = rows[path_id]
            writer.writerow({
                "candidate_id": path_id, "kind": kind, "matched_control_id": "",
                **{key: path[key] for key in ["cagr_percent", "mdd_percent", "vol_percent", "sharpe", "trades", "average_cash_ratio", "max_gross", "min_weight", "fingerprint", "source_events", "us_derisk_events", "available_us_derisk_events", "retained_events"]},
                "folds_sharpe_ge_matched": "CONTROL", "worst_fold_sharpe": min(path["fold_sharpes"]), "admit_for_robustness": "CONTROL",
            })
        writer.writerow({
            "candidate_id": CANDIDATE_ID, "kind": "FACTOR", "matched_control_id": MATCHED_CONTROL_ID,
            **{key: metrics[key] for key in ["cagr_percent", "mdd_percent", "vol_percent", "sharpe", "trades", "average_cash_ratio", "max_gross", "min_weight", "fingerprint", "source_events", "us_derisk_events", "available_us_derisk_events", "retained_events"]},
            "folds_sharpe_ge_matched": flags["folds_sharpe_ge_matched"], "worst_fold_sharpe": flags["worst_fold_sharpe"], "admit_for_robustness": flags["admit_for_robustness"],
        })
    return document
...
def main() -> int:
    ...
    env.update({
        ...
        "ATM_TIC_FOREIGN_EQUITY_FLOW_V1": "1",
        "ATM_TIC_FOREIGN_EQUITY_FLOW_V1_FACTOR_PATH": str(factor_input),
    })
    output_dir = Path(args.output_dir)
    if args.formal:
        env["ATM_TIC_FOREIGN_EQUITY_FLOW_V1_FORMAL"] = "1"
        env["ATM_TIC_FOREIGN_EQUITY_FLOW_V1_OUTPUT_DIR"] = str(output_dir)
    stdout = run([str(BINARY)], env=env, timeout=300)
    if "TIC_FOREIGN_EQUITY_FLOW_V1_COMPLETE" not in stdout:
        raise RuntimeError("Swift TIC foreign equity flow trial did not complete")
    if not args.formal:
        ...
        print("TIC_FOREIGN_EQUITY_FLOW_V1_SMOKE_OK")
        return 0
    ...
    print("TIC_FOREIGN_EQUITY_FLOW_V1_FORMAL_COMPLETE")
    return 0
