#!/usr/bin/env python3
"""Run ATM-SVP2-VRP-001 through the shared Swift engine."""
...
FRAGMENT = ROOT / "tools/vrp_proxy_v1.swiftpart"
LOGIC = ROOT / "tools/vrp_proxy_v1_logic.swift"
ASSEMBLED = Path("/private/tmp/atm_vrp_proxy_v1.swift")
BINARY = Path("/private/tmp/atm_vrp_proxy_v1")
CANDIDATE_ID = "F-VRP-PROXY"
MATCHED_CONTROL_ID = "C-VRP-ALWAYS"
...
def evaluate_candidate(metrics: dict, *, v11: dict, matched_control: dict) -> dict:
    ...
    checks = {
        "cagr_ge_v11_plus_0_5pp": float(metrics["cagr_percent"]) >= float(v11["cagr_percent"]) + 0.5,
...
def parse_candidate(block: str) -> dict:
    ...
    required = {
        ...
        "fingerprint", "source_events", "eligible_events", "available_events", "completed_events",
...
def validate_smoke(stdout: str) -> dict:
    ...
    ids = re.findall(r"^SMOKE_VRP_PATH=(.+)$", stdout, flags=re.MULTILINE)
    source_counts = [int(value) for value in re.findall(r"^SMOKE_VRP_SOURCE_COUNT=(\d+)$", stdout, flags=re.MULTILINE)]
    eligible_counts = [int(value) for value in re.findall(r"^SMOKE_VRP_ELIGIBLE_COUNT=(\d+)$", stdout, flags=re.MULTILINE)]
    available_counts = [int(value) for value in re.findall(r"^SMOKE_VRP_AVAILABLE_COUNT=(\d+)$", stdout, flags=re.MULTILINE)]
    ...
    if not fingerprint or not identity or identity.group(1) != "true":
        raise RuntimeError("smoke identity/fingerprint check failed")
    ...
    if len(available_counts) != 2 or len(set(available_counts)) != 1 or available_counts[0] <= 0:
        raise RuntimeError(f"available count mismatch: {available_counts}")
...
def write_outputs(output_dir: Path, rows: dict[str, dict]) -> dict:
    ...
    document = {
        ...
        "strategy_lineage": "variance-risk-premium-proxy-v1",
        "v11_control": v11,
        "matched_control": matched,
        "candidate_results": [candidate_result],
        "admitted_candidates": [CANDIDATE_ID] if candidate_result["admit_for_robustness"] else [],
    }
        ...
        for path_id, kind in [("V11-CONTROL", "CONTROL"), (MATCHED_CONTROL_ID, "MATCHED_CONTROL")]:
            ...
            writer.writerow({
                ...
                **{key: path[key] for key in [
                    ...
                    "max_gross", "min_weight", "fingerprint", "source_events", "eligible_events",
                    "available_events", "completed_events",
                ]},
...
def main() -> int:
    ...
    if not (factor_dir / "VIXCLS.csv").is_file():
        raise SystemExit(f"factor file missing: {factor_dir / 'VIXCLS.csv'}")
    ...
    env.update({
        ...
        "ATM_VRP_PROXY_V1": "1",
        "ATM_VRP_PROXY_V1_DATA_DIR": str(factor_dir),
    })
    output_dir = Path(args.output_dir)
    if args.formal:
        env["ATM_VRP_PROXY_V1_FORMAL"] = "1"
        env["ATM_VRP_PROXY_V1_OUTPUT_DIR"] = str(output_dir)
    stdout = run([str(BINARY)], env=env, timeout=300)
    if "VRP_PROXY_V1_COMPLETE" not in stdout:
        raise RuntimeError("Swift VRP proxy trial did not complete")
    ...
    print("VRP_PROXY_V1_FORMAL_COMPLETE")
    return 0
