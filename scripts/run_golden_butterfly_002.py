#!/usr/bin/env python3
"""Run the preregistered annual Golden Butterfly."""
...
TRIAL_ID = "ATM-SVP3-GOLDEN-BUTTERFLY-002"
CANDIDATE_ID = "S-GOLDEN-BUTTERFLY-002"
FRAGMENT = ROOT / "tools/golden_butterfly_002.swiftpart"
LOGIC = ROOT / "tools/golden_butterfly_001_logic.swift"
ASSEMBLED = Path("/private/tmp/atm_golden_butterfly_002.swift")
BINARY = Path("/private/tmp/atm_golden_butterfly_002")
...
def validate_document(document: dict) -> None:
    ...
    if document.get("evaluation_start") != "2004-01-30" or document.get("evaluation_end") != "2026-08-13":
        raise RuntimeError("evaluation window drifted")
    if abs(float(document.get("sleeve_weight", -1)) - 0.20) > 1e-12:
        raise RuntimeError("Golden Butterfly sleeve weight drifted")
...
def evaluate(document: dict) -> dict:
    ...
    objective_checks = {
        "cagr_ge_10pct": float(candidate["cagr_percent"]) >= 10.0,
    ...
    return {
        ...
        "validation_warnings": [
            ...
            "SPY, VBR and TLT use frozen total-return proxies; short Treasury is DTB3 synthetic accrual; gold is CNY",
        ],
        "diagnostics": {
...
def parse_formal_stdout(stdout: str) -> dict:
    prefix = "GOLDEN_BUTTERFLY_002_FORMAL_JSON="
    lines = [line[len(prefix):] for line in stdout.splitlines() if line.startswith(prefix)]
    if len(lines) != 1 or "GOLDEN_BUTTERFLY_002_FORMAL_OK" not in stdout:
...
def main() -> int:
    ...
    compile_binary()
    env = os.environ.copy()
    env.update({"ATM_HISTORY_FIXTURE": str(fixture), "ATM_GOLDEN_BUTTERFLY_002": "1"})
    if args.formal:
        env["ATM_GOLDEN_BUTTERFLY_002_FORMAL"] = "1"
        env["ATM_GOLDEN_BUTTERFLY_002_OUTPUT_DIR"] = str(Path(args.output_dir))
    stdout = run([str(BINARY)], env=env)
    if not args.formal:
        if "GOLDEN_BUTTERFLY_002_SMOKE_OK" not in stdout:
            print(stdout, end="")
    ...
    print("GOLDEN_BUTTERFLY_002_FORMAL_COMPLETE")
    return 0
