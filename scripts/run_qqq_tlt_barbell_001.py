#!/usr/bin/env python3
"""Run the preregistered annual QQQ/TLT growth-duration barbell."""
...
TRIAL_ID = "ATM-SVP3-QQQ-TLT-BARBELL-001"
CANDIDATE_ID = "S-QQQ-TLT-BARBELL-001"
FRAGMENT = ROOT / "tools/qqq_tlt_barbell_001.swiftpart"
LOGIC = ROOT / "tools/qqq_tlt_barbell_001_logic.swift"
ASSEMBLED = Path("/private/tmp/atm_qqq_tlt_barbell_001.swift")
BINARY = Path("/private/tmp/atm_qqq_tlt_barbell_001")
...
def validate_document(document: dict) -> None:
    ...
    if abs(float(document.get("qqq_weight", -1)) - 0.5) > 1e-12 or abs(float(document.get("tlt_weight", -1)) - 0.5) > 1e-12:
        raise RuntimeError("barbell weights drifted")
...
def evaluate(document: dict) -> dict:
    ...
    qqq = document["qqq_buy_hold_control"]
    ...
    comparison_checks = {
        "sharpe_gt_qqq_buy_hold": float(candidate["sharpe"]) > float(qqq["sharpe"]),
        "mdd_le_qqq_buy_hold": float(candidate["mdd_percent"]) <= float(qqq["mdd_percent"]),
    }
    ...
    return {
        ...
        "validation_warnings": [
            ...
            "QQQ and TLT are matched Yahoo adjusted-close total-return proxies, not raw price indices",
        ],
        "diagnostics": {
            "positive_fold_count": positive_folds,
            "fold_count": len(folds),
            "positive_fold_fraction": fraction,
            "qqq_buy_hold_cagr_percent": float(qqq["cagr_percent"]),
            "qqq_buy_hold_sharpe": float(qqq["sharpe"]),
            "qqq_buy_hold_mdd_percent": float(qqq["mdd_percent"]),
...
def write_outputs(output_dir: Path, document: dict, evaluation: dict) -> None:
    ...
    with (output_dir / "candidate-metrics.csv").open("w", encoding="utf-8", newline="") as handle:
        ...
        for kind, row in [
            ("CANDIDATE", document["candidate"]),
            ("QQQ_BUY_HOLD_CONTROL", document["qqq_buy_hold_control"]),
...
def parse_formal_stdout(stdout: str) -> dict:
    prefix = "QQQ_TLT_BARBELL_001_FORMAL_JSON="
    lines = [line[len(prefix):] for line in stdout.splitlines() if line.startswith(prefix)]
    if len(lines) != 1 or "QQQ_TLT_BARBELL_001_FORMAL_OK" not in stdout:
...
def main() -> int:
    ...
    compile_binary()
    env = os.environ.copy()
    env.update({"ATM_HISTORY_FIXTURE": str(fixture), "ATM_QQQ_TLT_BARBELL_001": "1"})
    if args.formal:
        env["ATM_QQQ_TLT_BARBELL_001_FORMAL"] = "1"
        env["ATM_QQQ_TLT_BARBELL_001_OUTPUT_DIR"] = str(Path(args.output_dir))
    stdout = run([str(BINARY)], env=env)
    if not args.formal:
        if "QQQ_TLT_BARBELL_001_SMOKE_OK" not in stdout:
            print(stdout, end="")
    ...
    print("QQQ_TLT_BARBELL_001_FORMAL_COMPLETE")
    return 0
