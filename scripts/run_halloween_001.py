#!/usr/bin/env python3
"""Run the preregistered S&P 500 Halloween seasonal strategy."""
...
FRAGMENT = ROOT / "tools/halloween_001.swiftpart"
LOGIC = ROOT / "tools/halloween_001_logic.swift"
ASSEMBLED = Path("/private/tmp/atm_halloween_001.swift")
BINARY = Path("/private/tmp/atm_halloween_001")
...
def evaluate(document: dict) -> dict:
    ...
    buy_hold = document["buy_hold_control"]
    ...
    comparison_checks = {
        "sharpe_gt_buy_hold": float(candidate["sharpe"]) > float(buy_hold["sharpe"]),
        "mdd_le_buy_hold": float(candidate["mdd_percent"]) <= float(buy_hold["mdd_percent"]),
    }
    ...
    return {
        ...
        "diagnostics": {
            ...
            "buy_hold_cagr_percent": float(buy_hold["cagr_percent"]),
            "buy_hold_sharpe": float(buy_hold["sharpe"]),
            "buy_hold_mdd_percent": float(buy_hold["mdd_percent"]),
...
def write_outputs(output_dir: Path, document: dict, evaluation: dict) -> None:
    ...
    with (output_dir / "candidate-metrics.csv").open("w", encoding="utf-8", newline="") as handle:
        fields = [
            ...
            "trades", "active_target_sessions", "average_cash_ratio", "max_gross", "minimum_cash", "target_fingerprint",
        ]
        writer = csv.DictWriter(handle, fieldnames=fields, lineterminator="\n")
        writer.writeheader()
        for kind, row in [
            ("CANDIDATE", document["candidate"]),
            ("BUY_HOLD_CONTROL", document["buy_hold_control"]),
...
def parse_formal_stdout(stdout: str) -> dict:
    prefix = "HALLOWEEN_001_FORMAL_JSON="
    lines = [line[len(prefix):] for line in stdout.splitlines() if line.startswith(prefix)]
    if len(lines) != 1 or "HALLOWEEN_001_FORMAL_OK" not in stdout:
...
def main() -> int:
    ...
    compile_binary()
    env = os.environ.copy()
    env.update({"ATM_HISTORY_FIXTURE": str(fixture), "ATM_HALLOWEEN_001": "1"})
    if args.formal:
        env["ATM_HALLOWEEN_001_FORMAL"] = "1"
        env["ATM_HALLOWEEN_001_OUTPUT_DIR"] = str(Path(args.output_dir))
    stdout = run([str(BINARY)], env=env)
    if not args.formal:
        if "HALLOWEEN_001_SMOKE_OK" not in stdout:
            print(stdout, end="")
    ...
    print("HALLOWEEN_001_FORMAL_COMPLETE")
    return 0
