#!/usr/bin/env python3
"""Frozen bootstrap and cumulative DSR audit for ATM-SVP2-VRP-001."""
...
def prior_post_protocol_sharpes() -> list[float]:
    ...
    if len(sharpes) != 24:
        raise RuntimeError(f"global DSR prior inventory must contain 24 candidates, got {len(sharpes)}")
    return sharpes


def global_dsr(candidate_dir: Path, candidate_sharpe: float) -> dict:
    prior = prior_post_protocol_sharpes()
    trial_sharpes = prior + [candidate_sharpe]
    if len(trial_sharpes) != 25:
        raise RuntimeError("global DSR inventory must contain 25 candidates")
...
def main() -> int:
    ...
    if result["candidate_id"] != CANDIDATE_ID:
        raise RuntimeError("candidate metrics does not contain frozen VRP proxy candidate")
    ...
    document = {
        ...
        "method": {
            ...
            "global_post_protocol_dsr_trial_count": 25,
        },
        "candidate_results": [{
            "candidate_id": CANDIDATE_ID,
    ...
    print("VRP_PROXY_V1_STATS_COMPLETE")
    return 0
