#!/usr/bin/env python3
"""Frozen bootstrap and cumulative DSR audit for ATM-SVP2-TIC-001."""
...
BATCH_SIZE = 128
CANDIDATE_ID = "F-TIC-FLOW-CONTRARIAN-US"
MATCHED_CONTROL_ID = "C-TIC-FLOW-ALWAYS"
PRIOR_RESULT_PATHS = [
...
def aligned_returns(paths: dict[str, Path]) -> tuple[list[str], dict[str, np.ndarray]]:
    ...
    return dates, {key: to_returns(np.asarray([values[day] for day in dates], dtype=np.float64)) for key, values in value_maps.items()}
...
def batch_metrics(returns: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    mdd = np.max(1.0 - wealth / np.maximum.accumulate(wealth, axis=1), axis=1)
...
def bootstrap(candidate_dir: Path) -> dict:
    ...
    c_cagrs, c_sharpes, c_mdds, m_sharpes, v_cagrs = [], [], [], [], []
    ...
    while remaining > 0:
        ...
        c_cagrs.append(c_cagr); c_sharpes.append(c_sharpe); c_mdds.append(c_mdd); m_sharpes.append(m_sharpe); v_cagrs.append(v_cagr)
        remaining -= batch
    cagr_c, sharpe_c, mdd_c = np.concatenate(c_cagrs), np.concatenate(c_sharpes), np.concatenate(c_mdds)
    sharpe_m, cagr_v = np.concatenate(m_sharpes), np.concatenate(v_cagrs)
    p_cagr = float(np.mean(cagr_c > cagr_v))
    p_sharpe = float(np.mean(sharpe_c > sharpe_m))
    med_cagr = float(np.median(cagr_c - cagr_v))
    med_sharpe = float(np.median(sharpe_c - sharpe_m))
    mdd_p975 = float(np.quantile(mdd_c, 0.975))
    checks = {
        "probability_cagr_gt_v11_ge_0_90": p_cagr >= 0.90,
        "probability_sharpe_gt_matched_ge_0_90": p_sharpe >= 0.90,
        "median_cagr_delta_gt_0": med_cagr > 0,
        "median_sharpe_delta_gt_0": med_sharpe > 0,
        "candidate_mdd_p975_le_0_15": mdd_p975 <= 0.15,
    }
    return {
        "candidate_id": CANDIDATE_ID,
        "sample": {"first_date": dates[0], "last_date": dates[-1], "aligned_portfolio_dates": len(dates), "daily_returns": n},
        "observed": {"candidate": performance_metrics(candidate), "matched_control": performance_metrics(matched), "v11": performance_metrics(v11)},
        "bootstrap": {
            "replicates": REPLICATES, "block_sessions": BLOCK_SESSIONS, "rng_seed": RNG_SEED,
            "probability_cagr_gt_v11": p_cagr, "probability_sharpe_gt_matched_control": p_sharpe,
            "median_candidate_minus_v11_cagr": med_cagr, "median_candidate_minus_matched_sharpe": med_sharpe,
            "candidate_mdd_p975": mdd_p975,
            "candidate_cagr_p025": float(np.quantile(cagr_c, 0.025)), "candidate_cagr_p50": float(np.quantile(cagr_c, 0.50)), "candidate_cagr_p975": float(np.quantile(cagr_c, 0.975)),
            "candidate_sharpe_p025": float(np.quantile(sharpe_c, 0.025)), "candidate_sharpe_p50": float(np.quantile(sharpe_c, 0.50)), "candidate_sharpe_p975": float(np.quantile(sharpe_c, 0.975)),
        },
        "bootstrap_robust_pass": all(checks.values()), "checks": checks,
    }
...
def prior_post_protocol_sharpes() -> list[float]:
    ...
    if len(sharpes) != 30:
        raise RuntimeError(f"global DSR prior inventory must contain 30 candidates, got {len(sharpes)}")
    return sharpes


def global_dsr(candidate_dir: Path, candidate_sharpe: float) -> dict:
    trial_sharpes = prior_post_protocol_sharpes() + [candidate_sharpe]
    if len(trial_sharpes) != 31:
        raise RuntimeError("global DSR inventory must contain 31 candidates")
...
def main() -> int:
    ...
    candidate_dir, output_dir = Path(args.candidate_dir), Path(args.output_dir)
    ...
    if result["candidate_id"] != CANDIDATE_ID:
        raise RuntimeError("candidate metrics does not contain frozen TIC flow candidate")
    ...
    document = {
        "protocol_id": "ATM-SVP-2", "trial_id": "ATM-SVP2-TIC-001",
        "method": {"bootstrap_sampling": "paired circular moving blocks", "bootstrap_block_sessions": BLOCK_SESSIONS, "bootstrap_replicates": REPLICATES, "bootstrap_rng_seed": RNG_SEED, "family_pbo": "not_applicable_single_candidate", "global_post_protocol_dsr_trial_count": 31},
        "candidate_results": [{"candidate_id": CANDIDATE_ID, "deterministic_admit": deterministic, "bootstrap": bootstrap_result, "global_post_protocol_dsr": dsr, "global_dsr_pass": dsr_pass, "robust_factor_pass": robust}],
    ...
    with (output_dir / "statistical-audit.csv").open("w", encoding="utf-8", newline="") as handle:
        ...
        writer = csv.DictWriter(handle, fieldnames=["candidate_id", "deterministic_admit", "bootstrap_pass", "global_dsr_probability", "global_dsr_pass", "robust_factor_pass", "probability_cagr_gt_v11", "probability_sharpe_gt_matched", "candidate_mdd_p975"])
        writer.writeheader()
        writer.writerow({"candidate_id": CANDIDATE_ID, "deterministic_admit": deterministic, "bootstrap_pass": bootstrap_result["bootstrap_robust_pass"], "global_dsr_probability": dsr["dsr_probability"], "global_dsr_pass": dsr_pass, "robust_factor_pass": robust, "probability_cagr_gt_v11": boot["probability_cagr_gt_v11"], "probability_sharpe_gt_matched": boot["probability_sharpe_gt_matched_control"], "candidate_mdd_p975": boot["candidate_mdd_p975"]})
    print(json.dumps(document, ensure_ascii=False, sort_keys=True))
    print("TIC_FOREIGN_EQUITY_FLOW_V1_STATS_COMPLETE")
    return 0
