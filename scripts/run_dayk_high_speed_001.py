#!/usr/bin/env python3
"""Run the preregistered 日K高速001 trial through the shared Swift research engine."""
...
TRIAL_ID = "ATM-SVP2-DAYK-HS-001"
CANDIDATE_ID = "S-DAYK-HIGH-SPEED-001"
FRAGMENT = ROOT / "tools/dayk_high_speed_001.swiftpart"
LOGIC = ROOT / "tools/dayk_high_speed_001_logic.swift"
ASSEMBLED = Path("/private/tmp/atm_dayk_high_speed_001.swift")
BINARY = Path("/private/tmp/atm_dayk_high_speed_001")
...
def validate_document(document: dict) -> None:
    ...
    expected_candidates = {"DKHS001-GOLD", "DKHS001-NASDAQ", "DKHS001-SP500", "DKHS001-COMBINED"}
    expected_controls = {"C-GAP-GOLD", "C-GAP-NASDAQ", "C-GAP-SP500", "C-GAP-COMBINED"}
...
def evaluate(document: dict) -> dict:
    combined = path_by_id(document, "candidate_paths", "DKHS001-COMBINED")
    matched = path_by_id(document, "matched_control_paths", "C-GAP-COMBINED")
    cash = document["cash_control"]
    stress = document["slippage_stress"]
    standalone = [
        path_by_id(document, "candidate_paths", "DKHS001-GOLD"),
        path_by_id(document, "candidate_paths", "DKHS001-NASDAQ"),
        path_by_id(document, "candidate_paths", "DKHS001-SP500"),
    ]
    ...
    checks = {
        ...
        "combined_sharpe_gt_matched_gap_control": float(combined["sharpe"]) > float(matched["sharpe"]),
...
def parse_formal_stdout(stdout: str) -> dict:
    prefix = "DAYK_HIGH_SPEED_001_FORMAL_JSON="
    lines = [line[len(prefix):] for line in stdout.splitlines() if line.startswith(prefix)]
    if len(lines) != 1 or "DAYK_HIGH_SPEED_001_FORMAL_OK" not in stdout:
...
def main() -> int:
    ...
    env.update({
        ...
        "ATM_DAYK_HIGH_SPEED_001": "1",
    })
    if args.formal:
        env["ATM_DAYK_HIGH_SPEED_001_FORMAL"] = "1"
        env["ATM_DAYK_HIGH_SPEED_001_OUTPUT_DIR"] = str(Path(args.output_dir))
    stdout = run([str(BINARY)], env=env)
    if not args.formal:
        if "DAYK_HIGH_SPEED_001_SMOKE_OK" not in stdout:
    ...
    print("DAYK_HIGH_SPEED_001_FORMAL_COMPLETE")
    return 0
