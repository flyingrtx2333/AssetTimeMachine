#!/usr/bin/env python3
"""Run the preregistered 日K高速003 trial through the shared Swift research engine."""
...
TRIAL_ID = "ATM-SVP2-DAYK-HS-003"
CANDIDATE_ID = "S-DAYK-HIGH-SPEED-003"
FRAGMENT = ROOT / "tools/dayk_high_speed_003.swiftpart"
LOGIC = ROOT / "tools/dayk_high_speed_003_logic.swift"
ASSEMBLED = Path("/private/tmp/atm_dayk_high_speed_003.swift")
BINARY = Path("/private/tmp/atm_dayk_high_speed_003")
...
def validate_document(document: dict) -> None:
    ...
    expected_candidates = {"DKHS003-GOLD", "DKHS003-NASDAQ", "DKHS003-SP500", "DKHS003-COMBINED"}
    expected_controls = {"C-PULLBACK-GOLD", "C-PULLBACK-NASDAQ", "C-PULLBACK-SP500", "C-PULLBACK-COMBINED"}
...
def evaluate(document: dict) -> dict:
    combined = path_by_id(document, "candidate_paths", "DKHS003-COMBINED")
    matched = path_by_id(document, "matched_control_paths", "C-PULLBACK-COMBINED")
    cash = document["cash_control"]
    stress = document["slippage_stress"]
    standalone = [
        path_by_id(document, "candidate_paths", "DKHS003-GOLD"),
        path_by_id(document, "candidate_paths", "DKHS003-NASDAQ"),
        path_by_id(document, "candidate_paths", "DKHS003-SP500"),
    ]
    ...
    checks = {
        ...
        "combined_sharpe_gt_matched_pullback_control": float(combined["sharpe"]) > float(matched["sharpe"]),
...
def parse_formal_stdout(stdout: str) -> dict:
    prefix = "DAYK_HIGH_SPEED_003_FORMAL_JSON="
    lines = [line[len(prefix):] for line in stdout.splitlines() if line.startswith(prefix)]
    if len(lines) != 1 or "DAYK_HIGH_SPEED_003_FORMAL_OK" not in stdout:
...
def main() -> int:
    ...
    env.update({
        ...
        "ATM_DAYK_HIGH_SPEED_003": "1",
    })
    if args.formal:
        env["ATM_DAYK_HIGH_SPEED_003_FORMAL"] = "1"
        env["ATM_DAYK_HIGH_SPEED_003_OUTPUT_DIR"] = str(Path(args.output_dir))
    stdout = run([str(BINARY)], env=env)
    if not args.formal:
        if "DAYK_HIGH_SPEED_003_SMOKE_OK" not in stdout:
    ...
    print("DAYK_HIGH_SPEED_003_FORMAL_COMPLETE")
    return 0
