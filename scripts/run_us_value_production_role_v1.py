...
FRAGMENT = ROOT / "tools/us_value_production_role_v1.swiftpart"
ASSEMBLED = Path("/private/tmp/atm_us_value_production_role_v1.swift")
BINARY = Path("/private/tmp/atm_us_value_production_role_v1")
CANDIDATES = ("S-IWD-PROD-SP500-ROLE", "S-VBR-PROD-SP500-ROLE")
FROZEN_V11_FINGERPRINT = "ba67c8aa24bc7168"
EVALUATION_START = "2004-02-02"
FOLD_COUNT = 7
...
def write_outputs(output_dir: Path, rows: dict[str, dict]) -> dict:
    ...
    document = {
        "protocol_id": "ATM-SVP-2", "trial_id": "ATM-SVP2-US-VALUE-PROD-001", "strategy_lineage": "us-value-production-role-v1",
...
def main() -> int:
    parser=argparse.ArgumentParser(); parser.add_argument("--fixture",required=True); parser.add_argument("--iwd-input",required=True); parser.add_argument("--vbr-input",required=True); parser.add_argument("--output-dir",required=True); parser.add_argument("--formal",action="store_true"); args=parser.parse_args()
    ...
    compile_binary(); env=os.environ.copy(); env.update({"ATM_HISTORY_FIXTURE":args.fixture,"ATM_US_VALUE_PRODUCTION_ROLE_V1":"1","ATM_US_VALUE_PRODUCTION_ROLE_V1_IWD_PATH":args.iwd_input,"ATM_US_VALUE_PRODUCTION_ROLE_V1_VBR_PATH":args.vbr_input}); output=Path(args.output_dir)
    if args.formal: env["ATM_US_VALUE_PRODUCTION_ROLE_V1_FORMAL"]="1"; env["ATM_US_VALUE_PRODUCTION_ROLE_V1_OUTPUT_DIR"]=str(output)
    stdout=run([str(BINARY)],env=env,timeout=300)
    if "US_VALUE_PRODUCTION_ROLE_V1_COMPLETE" not in stdout: raise RuntimeError("Swift value production-path role trial did not complete")
    if not args.formal:
        smoke=validate_smoke(stdout); print(json.dumps({"mode":"SMOKE_NO_PERFORMANCE",**smoke},ensure_ascii=False,sort_keys=True)); print("US_VALUE_PRODUCTION_ROLE_V1_SMOKE_OK"); return 0
    ...
    document=write_outputs(output,parse_formal(stdout)); print(json.dumps(document,ensure_ascii=False,sort_keys=True)); print("US_VALUE_PRODUCTION_ROLE_V1_FORMAL_COMPLETE"); return 0
