#!/usr/bin/env python3
"""Run the preregistered one-month cross-asset short-term momentum strategy."""
...
FRAGMENT = ROOT / "tools/short_momentum_001.swiftpart"
LOGIC = ROOT / "tools/short_momentum_001_logic.swift"
ASSEMBLED = Path("/private/tmp/atm_short_momentum_001.swift")
BINARY = Path("/private/tmp/atm_short_momentum_001")
...
def validate_document(document: dict) -> None:
    ...
    if document.get("evaluation_start") != "2002-07-03" or document.get("evaluation_end") != "2026-08-20":
...
def parse_formal_stdout(stdout: str) -> dict:
    prefix = "SHORT_MOMENTUM_001_FORMAL_JSON="
    lines = [line[len(prefix):] for line in stdout.splitlines() if line.startswith(prefix)]
    if len(lines) != 1 or "SHORT_MOMENTUM_001_FORMAL_OK" not in stdout:
...
def main() -> int:
    ...
    compile_binary()
    env = os.environ.copy()
    env.update({"ATM_HISTORY_FIXTURE": str(fixture), "ATM_SHORT_MOMENTUM_001": "1"})
    if args.formal:
        env["ATM_SHORT_MOMENTUM_001_FORMAL"] = "1"
        env["ATM_SHORT_MOMENTUM_001_OUTPUT_DIR"] = str(Path(args.output_dir))
    stdout = run([str(BINARY)], env=env)
    if not args.formal:
        if "SHORT_MOMENTUM_001_SMOKE_OK" not in stdout:
            print(stdout, end="")
    ...
    print("SHORT_MOMENTUM_001_FORMAL_COMPLETE")
    return 0
