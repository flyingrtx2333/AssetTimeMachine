#!/usr/bin/env python3
"""Run preregistered MTUM/QUAL role substitutions on the frozen production V11 target path."""
...
FRAGMENT = ROOT / "tools/us_momentum_quality_role_v1.swiftpart"
ASSEMBLED = Path("/private/tmp/atm_us_mq_role_v1.swift")
BINARY = Path("/private/tmp/atm_us_mq_role_v1")
...
def evaluate(metrics: dict, v11: dict) -> dict:
    ...
    constraints = metrics["max_gross"] <= 1.000000001 and metrics["min_weight"] >= -1e-10 and metrics["fingerprint"] == FROZEN_V11_FINGERPRINT
    checks = {
...
def validate_smoke(stdout: str) -> dict:
    ...
    if not identity or identity.group(1) != "true" or not fingerprint or fingerprint.group(1) != FROZEN_V11_FINGERPRINT or not events or not start or start.group(1) != EVALUATION_START:
        raise RuntimeError("production-path smoke identity failed")
    if ids != list(CANDIDATES) or target_fps != [FROZEN_V11_FINGERPRINT, FROZEN_V11_FINGERPRINT]:
        raise RuntimeError(f"candidate target-path drift: ids={ids} fps={target_fps}")
...
def write_outputs(output_dir: Path, rows: dict[str, dict]) -> dict:
    ...
    document = {
        "protocol_id": "ATM-SVP-2", "trial_id": "ATM-SVP2-US-MQ-ROLE-001", "strategy_lineage": "us-momentum-quality-role-v1",
...
def main() -> int:
    ...
    compile_binary(); env=os.environ.copy(); env.update({"ATM_HISTORY_FIXTURE":args.fixture,"ATM_US_MQ_ROLE_V1":"1","ATM_US_MQ_ROLE_V1_MTUM_PATH":args.mtum_input,"ATM_US_MQ_ROLE_V1_QUAL_PATH":args.qual_input}); output=Path(args.output_dir)
    if args.formal: env["ATM_US_MQ_ROLE_V1_FORMAL"]="1"; env["ATM_US_MQ_ROLE_V1_OUTPUT_DIR"]=str(output)
    stdout=run([str(BINARY)],env=env,timeout=300)
    if "US_MQ_ROLE_V1_COMPLETE" not in stdout: raise RuntimeError("Swift momentum/quality role trial did not complete")
    if not args.formal:
        smoke=validate_smoke(stdout); print(json.dumps({"mode":"SMOKE_NO_PERFORMANCE",**smoke},ensure_ascii=False,sort_keys=True)); print("US_MQ_ROLE_V1_SMOKE_OK"); return 0
    document=write_outputs(output,parse_formal(stdout)); print(json.dumps(document,ensure_ascii=False,sort_keys=True)); print("US_MQ_ROLE_V1_FORMAL_COMPLETE"); return 0
