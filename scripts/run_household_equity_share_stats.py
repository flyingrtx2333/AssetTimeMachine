#!/usr/bin/env python3
"""Frozen bootstrap and cumulative DSR audit for ATM-SVP2-HESHARE-001."""
...
def prior_post_protocol_sharpes() -> list[float]:
    ...
    if len(sharpes) != 31:
        raise RuntimeError(f"global DSR prior inventory must contain 31 candidates, got {len(sharpes)}")
    return sharpes


def global_dsr(candidate_dir: Path, candidate_sharpe: float) -> dict:
    trial_sharpes = prior_post_protocol_sharpes() + [candidate_sharpe]
    if len(trial_sharpes) != 32:
        raise RuntimeError("global DSR inventory must contain 32 candidates")
...
def main() -> int:
    ...
    if result["candidate_id"] != CANDIDATE_ID:
        raise RuntimeError("candidate metrics does not contain frozen HEShare candidate")
    ...
    document = {
        ...
        "method": {
            ...
            "global_post_protocol_dsr_trial_count": 32,
        },
        "candidate_results": [{
            "candidate_id": CANDIDATE_ID,
    ...
    print("HOUSEHOLD_EQUITY_SHARE_V1_STATS_COMPLETE")
    return 0
