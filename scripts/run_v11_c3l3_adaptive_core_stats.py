...
CANDIDATE_ID = "S-V11-C3L3-CORE-SWITCH"
...
PRIOR_RESULT_PATHS = [
    ...
    ROOT / "tools/research-results/strategy-validation/results/ATM-SVP2-V11-HIGHCORE-001.json",
]
...
def prior_sharpes() -> list[float]:
    ...
    for path in PRIOR_RESULT_PATHS:
        ...
        for result in document.get("candidate_results") or []:
            ...
            values.append(value)
    if len(values) != 43:
        raise RuntimeError(f"expected 43 prior performance-bearing candidates, got {len(values)}")
    return values
...
def main() -> int:
    ...
    if len(all_sharpes) != 44:
        raise RuntimeError("global DSR inventory must contain 44 candidates")
    ...
    document = {
        ...
        "trial_id": "ATM-SVP2-V11-C3L3-CORE-SWITCH-001",
        "method": {
            "bootstrap_sampling": "paired circular moving blocks C3/L3 adaptive-core versus frozen V11",
            "bootstrap_block_sessions": BLOCK_SESSIONS,
            "bootstrap_replicates": REPLICATES,
            "bootstrap_rng_seed": RNG_SEED,
            "family_pbo": "not_applicable_single_fixed_candidate",
            "global_post_protocol_dsr_prior_trial_count": 43,
            "global_post_protocol_dsr_total_trial_count": 44,
    ...
    print("V11_C3L3_ADAPTIVE_CORE_V1_STATS_COMPLETE")
    return 0
