#!/usr/bin/env python3
"""Run ATM-SVP2-LIT-STRESS-001 through the shared Swift engine."""
...
FRAGMENT = ROOT / "tools/literature_stress_factor_v1.swiftpart"
LOGIC = ROOT / "tools/literature_stress_factor_v1_logic.swift"
ASSEMBLED = Path("/private/tmp/atm_literature_stress_factor_v1.swift")
BINARY = Path("/private/tmp/atm_literature_stress_factor_v1")
FORMAL_CANDIDATES = ["F-BAA", "F-EPU", "F-MOVE"]
MATCHED_CONTROLS = {
    "F-BAA": "C-BAA-ALWAYS",
    "F-EPU": "C-EPU-ALWAYS",
...
def validate_smoke(stdout: str) -> dict:
    ...
    ids = re.findall(r"^SMOKE_LIT_PATH=(.+)$", stdout, flags=re.MULTILINE)
    source_counts = [int(value) for value in re.findall(r"^SMOKE_LIT_SOURCE_COUNT=(\d+)$", stdout, flags=re.MULTILINE)]
    derisk_counts = [int(value) for value in re.findall(r"^SMOKE_LIT_DERISK_COUNT=(\d+)$", stdout, flags=re.MULTILINE)]
...
def write_outputs(output_dir: Path, rows: dict[str, dict]) -> dict:
    ...
    document = {
        ...
        "strategy_lineage": "literature-stress-factor-family-v1",
        "v11_control": v11,
    ...
    with (output_dir / "candidate-metrics.csv").open("w", encoding="utf-8", newline="") as handle:
        ...
        writer.writeheader()
        for path_id in ["V11-CONTROL", *MATCHED_CONTROLS.values()]:
            metrics = rows[path_id]
            writer.writerow({
                "candidate_id": path_id,
                "kind": "CONTROL" if path_id == "V11-CONTROL" else "MATCHED_CONTROL",
                "matched_control_id": "",
...
def main() -> int:
    ...
    for name in ["BAA.csv", "EPU.csv", "MOVE.csv"]:
    ...
    env.update({
        ...
        "ATM_LITERATURE_STRESS_FACTOR_V1": "1",
        "ATM_LITERATURE_STRESS_FACTOR_V1_DATA_DIR": str(factor_dir),
    })
    output_dir = Path(args.output_dir)
    if args.formal:
        env["ATM_LITERATURE_STRESS_FACTOR_V1_FORMAL"] = "1"
        env["ATM_LITERATURE_STRESS_FACTOR_V1_OUTPUT_DIR"] = str(output_dir)
    stdout = run([str(BINARY)], env=env, timeout=300)
    if "LITERATURE_STRESS_FACTOR_V1_COMPLETE" not in stdout:
        raise RuntimeError("Swift literature stress-factor family did not complete")
    ...
    if not args.formal:
        ...
        print("LITERATURE_STRESS_FACTOR_V1_SMOKE_OK")
        return 0
    ...
    print("LITERATURE_STRESS_FACTOR_V1_FORMAL_COMPLETE")
    return 0
