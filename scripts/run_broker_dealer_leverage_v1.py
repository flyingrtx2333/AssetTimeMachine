#!/usr/bin/env python3
"""Run preregistered ATM-SVP2-BDLEV-001 through the shared Swift engine."""
...
FRAGMENT = ROOT / "tools/broker_dealer_leverage_v1.swiftpart"
LOGIC = ROOT / "tools/broker_dealer_leverage_v1_logic.swift"
ASSEMBLED = Path("/private/tmp/atm_broker_dealer_leverage_v1.swift")
BINARY = Path("/private/tmp/atm_broker_dealer_leverage_v1")
CANDIDATE_ID = "F-BDLEV-ANNUAL-US"
MATCHED_CONTROL_ID = "C-BDLEV-ALWAYS"
...
def validate_smoke(stdout: str) -> dict:
    ...
    ids = re.findall(r"^SMOKE_BDLEV_PATH=(.+)$", stdout, flags=re.MULTILINE)
    source_counts = [int(value) for value in re.findall(r"^SMOKE_BDLEV_SOURCE_COUNT=(\d+)$", stdout, flags=re.MULTILINE)]
    derisk_counts = [int(value) for value in re.findall(r"^SMOKE_BDLEV_US_DERISK_COUNT=(\d+)$", stdout, flags=re.MULTILINE)]
    available_counts = [int(value) for value in re.findall(r"^SMOKE_BDLEV_AVAILABLE_COUNT=(\d+)$", stdout, flags=re.MULTILINE)]
...
def write_outputs(output_dir: Path, rows: dict[str, dict]) -> dict:
    ...
    document = {
        ...
        "trial_id": "ATM-SVP2-BDLEV-001",
        "strategy_lineage": "broker-dealer-annual-book-leverage-v1",
        "v11_control": v11,
...
def main() -> int:
    ...
    env.update({
        ...
        "ATM_BROKER_DEALER_LEVERAGE_V1": "1",
        "ATM_BROKER_DEALER_LEVERAGE_V1_FACTOR_PATH": str(factor_input),
    })
    output_dir = Path(args.output_dir)
    if args.formal:
        env["ATM_BROKER_DEALER_LEVERAGE_V1_FORMAL"] = "1"
        env["ATM_BROKER_DEALER_LEVERAGE_V1_OUTPUT_DIR"] = str(output_dir)
    stdout = run([str(BINARY)], env=env, timeout=300)
    if "BROKER_DEALER_LEVERAGE_V1_COMPLETE" not in stdout:
        raise RuntimeError("Swift broker-dealer leverage trial did not complete")
    ...
    if not args.formal:
        ...
        print("BROKER_DEALER_LEVERAGE_V1_SMOKE_OK")
        return 0
    ...
    print("BROKER_DEALER_LEVERAGE_V1_FORMAL_COMPLETE")
    return 0
