#!/usr/bin/env python3
"""Fetch fixed U.S. value-role total-return inputs without computing strategy performance."""
...
SOURCES = {
    "IWD": {"series_id": "IWD", "official_inception": "2000-05-22", "role": "Russell 1000 Value / large-mid value"},
    "VBR": {"series_id": "VBR", "official_inception": "2004-01-26", "role": "U.S. small-cap value"},
}
USER_AGENT = "AssetTimeMachine-USValueRoleV1/1.0"
...
def fetch_adjusted_close(series_id: str, start: str, end: str) -> tuple[list[tuple[str, float]], dict[str, Any]]:
    ...
    if not rows or len({day for day, _ in rows}) != len(rows):
        raise RuntimeError(f"invalid adjusted-close rows for {series_id}")
...
def main() -> int:
    ...
    summary: dict[str, Any] = {
        "purpose": "fixed IWD/VBR role-substitution data construction only; no strategy performance",
        "performance_computed": False,
        "selection_policy": "both externally motivated value-role candidates are fetched before any AssetTimeMachine candidate performance is viewed",
        "series": {},
    }
    for name, spec in SOURCES.items():
        rows, provenance = fetch_adjusted_close(spec["series_id"], spec["official_inception"], args.end)
        if rows[0][0] < spec["official_inception"]:
            raise RuntimeError(f"{name} begins before declared official inception")
    ...
    summary["common_evaluation_start"] = max(item["first_date"] for item in summary["series"].values())
    summary_path = output_dir / "fetch-summary.json"
    summary_path.write_text(json.dumps(summary, ensure_ascii=False, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    print(json.dumps(summary, ensure_ascii=False, sort_keys=True))
    print("US_VALUE_ROLE_V1_FETCH_COMPLETE")
    return 0
