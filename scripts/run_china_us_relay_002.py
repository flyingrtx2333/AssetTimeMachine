#!/usr/bin/env python3
"""Run the preregistered China-close -> U.S.-intraday relay strategy."""
...
TRIAL_ID = "ATM-SVP3-CHINA-US-RELAY-002"
CANDIDATE_ID = "S-CHINA-US-RELAY-002"
FRAGMENT = ROOT / "tools/china_us_relay_002.swiftpart"
LOGIC = ROOT / "tools/china_us_relay_002_logic.swift"
ASSEMBLED = Path("/private/tmp/atm_china_us_relay_002.swift")
BINARY = Path("/private/tmp/atm_china_us_relay_002")
...
def validate_document(document: dict) -> None:
    ...
    if document.get("evaluation_start") != "2005-01-05" or document.get("evaluation_end") != "2026-08-20":
    ...
    if candidate_ids != {"CUS002-NASDAQ", "CUS002-SP500", "CUS002-COMBINED"}:
        raise RuntimeError(f"candidate path mismatch: {sorted(candidate_ids)}")
    if control_ids != {"C-CHINA-AVAILABLE-NASDAQ", "C-CHINA-AVAILABLE-SP500", "C-CHINA-AVAILABLE-COMBINED"}:
        raise RuntimeError(f"control path mismatch: {sorted(control_ids)}")


def evaluate(document: dict) -> dict:
    combined = path_by_id(document, "candidate_paths", "CUS002-COMBINED")
    nasdaq = path_by_id(document, "candidate_paths", "CUS002-NASDAQ")
    sp500 = path_by_id(document, "candidate_paths", "CUS002-SP500")
    matched = path_by_id(document, "matched_control_paths", "C-CHINA-AVAILABLE-COMBINED")
    ...
    sentinel_take_profit_hits = sum(int(value) for value in (combined.get("take_profit_hits_by_symbol") or {}).values())

    constraints = {
        "actual_round_trips_gt_0": int(trade_stats["round_trips"]) > 0,
        "sentinel_take_profit_hits_eq_0": sentinel_take_profit_hits == 0,
        "max_gross_le_1": float(combined["max_intraday_gross"]) <= 1.000000001,
        "cash_nonnegative": float(combined["minimum_end_of_day_cash"]) >= -1e-8,
        "no_overnight_positions": int(combined["overnight_position_days"]) == 0,
    }
    validation_checks = {
    ...
    objective_checks = {
        ...
        "combined_profit_factor_ge_1_1": float(trade_stats["profit_factor"]) >= 1.10,
    ...
    comparison_checks = {
        "candidate_sharpe_gt_matched_availability_control": float(combined["sharpe"]) > float(matched["sharpe"]),
        "candidate_cagr_gt_matched_availability_control": float(combined["cagr_percent"]) > float(matched["cagr_percent"]),
        "candidate_mdd_le_matched_availability_control": float(combined["mdd_percent"]) <= float(matched["mdd_percent"]),
    }
    ...
    return {
        ...
        "diagnostics": {
            ...
            "matched_control_cagr_percent": float(matched["cagr_percent"]),
            "matched_control_sharpe": float(matched["sharpe"]),
            "matched_control_mdd_percent": float(matched["mdd_percent"]),
        },
    }
...
def write_outputs(output_dir: Path, document: dict, evaluation: dict) -> None:
    ...
    with (output_dir / "candidate-metrics.csv").open("w", encoding="utf-8", newline="") as handle:
        fields = [
            ...
            "active_sessions", "max_intraday_gross", "overnight_position_days", "target_fingerprint",
        ]
        ...
        for kind, row in rows:
            ...
            writer.writerow({
                ...
                "max_intraday_gross": row["max_intraday_gross"],
                "overnight_position_days": row["overnight_position_days"],
                "target_fingerprint": row["target_fingerprint"],
            })


def parse_formal_stdout(stdout: str) -> dict:
    prefix = "CHINA_US_RELAY_002_FORMAL_JSON="
    lines = [line[len(prefix):] for line in stdout.splitlines() if line.startswith(prefix)]
    if len(lines) != 1 or "CHINA_US_RELAY_002_FORMAL_OK" not in stdout:
...
def main() -> int:
    ...
    compile_binary()
    env = os.environ.copy()
    env.update({"ATM_HISTORY_FIXTURE": str(fixture), "ATM_CHINA_US_RELAY_002": "1"})
    if args.formal:
        env["ATM_CHINA_US_RELAY_002_FORMAL"] = "1"
        env["ATM_CHINA_US_RELAY_002_OUTPUT_DIR"] = str(Path(args.output_dir))
    stdout = run([str(BINARY)], env=env)
    if not args.formal:
        if "CHINA_US_RELAY_002_SMOKE_OK" not in stdout:
    ...
    print("CHINA_US_RELAY_002_FORMAL_COMPLETE")
    return 0
