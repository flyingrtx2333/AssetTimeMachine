#!/usr/bin/env python3
"""Run preregistered Household Equity Share V1 through the shared Swift App engine."""
...
FRAGMENT = ROOT / "tools/household_equity_share_v1.swiftpart"
LOGIC = ROOT / "tools/household_equity_share_v1_logic.swift"
ASSEMBLED = Path("/private/tmp/atm_household_equity_share_v1.swift")
BINARY = Path("/private/tmp/atm_household_equity_share_v1")
CANDIDATE_ID = "F-HESHARE-US-COMPLETION"
MATCHED_CONTROL_ID = "C-HESHARE-US-ALWAYS"
...
def evaluate_candidate(metrics: dict, *, v11: dict, matched_control: dict) -> dict:
    ...
    if len(candidate_folds) != FOLD_COUNT or len(matched_folds) != FOLD_COUNT:
        raise ValueError("candidate and matched control must both have five post-stable fold Sharpes")
    ...
    checks = {
        "evaluation_cagr_gt_v11": float(metrics["evaluation_cagr_percent"]) > float(v11["evaluation_cagr_percent"]),
        "evaluation_sharpe_ge_v11": float(metrics["evaluation_sharpe"]) >= float(v11["evaluation_sharpe"]),
        "evaluation_sharpe_gt_matched_control": float(metrics["evaluation_sharpe"]) > float(matched_control["evaluation_sharpe"]),
        "evaluation_mdd_le_10pct": float(metrics["evaluation_mdd_percent"]) <= 10.0,
        "folds_sharpe_ge_matched_ge_4_of_5": fold_wins >= 4,
...
def parse_candidate(block: str) -> dict:
    ...
    for line in lines[1:]:
        if "=" in line:
            ...
            values[key] = value
    required = {
        "cagr", "mdd", "vol", "sharpe", "evaluation_cagr", "evaluation_mdd", "evaluation_sharpe",
        "trades", "average_cash", "max_gross", "min_weight", "fingerprint",
        "source_events", "eligible_events", "available_events", "completed_events",
    ...
    return {
        ...
        "available_events": int(values["available_events"]),
        "completed_events": int(values["completed_events"]),
...
def validate_smoke(stdout: str) -> dict:
    ...
    ids = re.findall(r"^SMOKE_HESHARE_PATH=(.+)$", stdout, flags=re.MULTILINE)
    source_counts = [int(value) for value in re.findall(r"^SMOKE_HESHARE_SOURCE_COUNT=(\d+)$", stdout, flags=re.MULTILINE)]
    eligible_counts = [int(value) for value in re.findall(r"^SMOKE_HESHARE_ELIGIBLE_COUNT=(\d+)$", stdout, flags=re.MULTILINE)]
    available_counts = [int(value) for value in re.findall(r"^SMOKE_HESHARE_AVAILABLE_COUNT=(\d+)$", stdout, flags=re.MULTILINE)]
    ...
    if len(eligible_counts) != 2 or len(set(eligible_counts)) != 1 or eligible_counts[0] <= 0:
        raise RuntimeError(f"eligible count mismatch: {eligible_counts}")
    ...
    return {
        ...
        "eligible_events": eligible_counts[0],
        "available_events": available_counts[0],
...
def write_outputs(output_dir: Path, rows: dict[str, dict]) -> dict:
    ...
    document = {
        ...
        "trial_id": "ATM-SVP2-HESHARE-001",
        "strategy_lineage": "household-equity-share-v1",
        "evaluation_window_start": "2017-01-01",
        "v11_control": v11,
    ...
    fields = [
        "candidate_id", "kind", "matched_control_id", "cagr_percent", "mdd_percent", "sharpe",
        "evaluation_cagr_percent", "evaluation_mdd_percent", "evaluation_sharpe", "trades",
        "average_cash_ratio", "max_gross", "min_weight", "source_events", "eligible_events",
        "available_events", "completed_events", "folds_sharpe_ge_matched", "worst_fold_sharpe",
        ...
        for path_id, kind in [("V11-CONTROL", "CONTROL"), (MATCHED_CONTROL_ID, "MATCHED_CONTROL")]:
            ...
            writer.writerow({
                ...
                **{key: path[key] for key in [
                    "cagr_percent", "mdd_percent", "sharpe", "evaluation_cagr_percent", "evaluation_mdd_percent",
                    "evaluation_sharpe", "trades", "average_cash_ratio", "max_gross", "min_weight", "source_events",
                    "eligible_events", "available_events", "completed_events",
                ]},
...
def main() -> int:
    ...
    env.update({
        ...
        "ATM_HOUSEHOLD_EQUITY_SHARE_V1": "1",
        "ATM_HOUSEHOLD_EQUITY_SHARE_V1_FACTOR_PATH": str(factor_input),
    })
    output_dir = Path(args.output_dir)
    if args.formal:
        env["ATM_HOUSEHOLD_EQUITY_SHARE_V1_FORMAL"] = "1"
        env["ATM_HOUSEHOLD_EQUITY_SHARE_V1_OUTPUT_DIR"] = str(output_dir)
    stdout = run([str(BINARY)], env=env, timeout=300)
    if "HOUSEHOLD_EQUITY_SHARE_V1_COMPLETE" not in stdout:
        raise RuntimeError("Swift Household Equity Share trial did not complete")
    ...
    if not args.formal:
        ...
        print("HOUSEHOLD_EQUITY_SHARE_V1_SMOKE_OK")
        return 0
    ...
    print("HOUSEHOLD_EQUITY_SHARE_V1_FORMAL_COMPLETE")
    return 0
