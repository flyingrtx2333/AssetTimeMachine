#!/usr/bin/env python3
"""Run preregistered Orthogonal Event Factor Family V3 through the shared Swift engine."""
...
FRAGMENT = ROOT / "tools/orthogonal_event_factor_v3.swiftpart"
LOGIC = ROOT / "tools/orthogonal_event_factor_v3_logic.swift"
ASSEMBLED = Path("/private/tmp/atm_orthogonal_event_factor_v3.swift")
BINARY = Path("/private/tmp/atm_orthogonal_event_factor_v3")
FORMAL_CANDIDATES = ["F-CREDIT", "F-CYCLICAL", "F-BREADTH"]
MATCHED_CONTROLS = {
...
def validate_smoke(stdout: str) -> dict:
    ...
    expected_ids = [
        "C-CREDIT-ALWAYS", "F-CREDIT",
        "C-CYCLICAL-ALWAYS", "F-CYCLICAL",
        "C-BREADTH-ALWAYS", "F-BREADTH",
    ]
...
def write_outputs(output_dir: Path, rows: dict[str, dict]) -> dict:
    ...
    document = {
        ...
        "trial_id": "ATM-SVP2-ORTHO-FACTOR-003",
        "strategy_lineage": "orthogonal-event-factor-family-v3",
        "v11_control": v11,
...
def main() -> int:
    ...
    for name in ["HYG.csv", "LQD.csv", "XLY.csv", "XLP.csv", "RSP.csv", "SPY.csv"]:
    ...
    env.update({
        ...
        "ATM_ORTHOGONAL_EVENT_FACTOR_V3": "1",
        "ATM_ORTHOGONAL_EVENT_FACTOR_V3_DATA_DIR": str(factor_dir),
    })
    output_dir = Path(args.output_dir)
    if args.formal:
        env["ATM_ORTHOGONAL_EVENT_FACTOR_V3_FORMAL"] = "1"
        env["ATM_ORTHOGONAL_EVENT_FACTOR_V3_OUTPUT_DIR"] = str(output_dir)
    stdout = run([str(BINARY)], env=env, timeout=300)
    if "ORTHOGONAL_EVENT_FACTOR_V3_COMPLETE" not in stdout:
    ...
    if not args.formal:
        ...
        print("ORTHOGONAL_EVENT_FACTOR_V3_SMOKE_OK")
        return 0
    ...
    print("ORTHOGONAL_EVENT_FACTOR_V3_FORMAL_COMPLETE")
    return 0
