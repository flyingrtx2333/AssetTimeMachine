#!/usr/bin/env python3
"""Run the preregistered annual Factor Duration."""
...
TRIAL_ID = "ATM-SVP3-FACTOR-DURATION-001"
CANDIDATE_ID = "S-FACTOR-DURATION-001"
FRAGMENT = ROOT / "tools/factor_duration_001.swiftpart"
LOGIC = ROOT / "tools/factor_duration_001_logic.swift"
ASSEMBLED = Path("/private/tmp/atm_factor_duration_001.swift")
BINARY = Path("/private/tmp/atm_factor_duration_001")
...
def validate_document(document: dict) -> None:
    ...
    if document.get("evaluation_start") != "2013-07-18" or document.get("evaluation_end") != "2026-08-13":
        raise RuntimeError("evaluation window drifted")
    if abs(float(document.get("sleeve_weight", -1)) - 0.25) > 1e-12:
        raise RuntimeError("Factor Duration sleeve weight drifted")
...
def evaluate(document: dict) -> dict:
    ...
    return {
        ...
        "validation_warnings": [
            ...
            "VBR, MTUM, QUAL, TLT and SPY use frozen adjusted-close total-return proxies with the same historical USD/CNY conversion",
        ],
        "diagnostics": {
...
def parse_formal_stdout(stdout: str) -> dict:
    prefix = "FACTOR_DURATION_001_FORMAL_JSON="
    lines = [line[len(prefix):] for line in stdout.splitlines() if line.startswith(prefix)]
    if len(lines) != 1 or "FACTOR_DURATION_001_FORMAL_OK" not in stdout:
...
def main() -> int:
    ...
    compile_binary()
    env = os.environ.copy()
    env.update({"ATM_HISTORY_FIXTURE": str(fixture), "ATM_FACTOR_DURATION_001": "1"})
    if args.formal:
        env["ATM_FACTOR_DURATION_001_FORMAL"] = "1"
        env["ATM_FACTOR_DURATION_001_OUTPUT_DIR"] = str(Path(args.output_dir))
    stdout = run([str(BINARY)], env=env)
    if not args.formal:
        if "FACTOR_DURATION_001_SMOKE_OK" not in stdout:
            print(stdout, end="")
    ...
    print("FACTOR_DURATION_001_FORMAL_COMPLETE")
    return 0
