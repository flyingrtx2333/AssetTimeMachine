#!/usr/bin/env python3
"""Run preregistered Orthogonal Event Budget Family V5 through the shared Swift engine."""
...
FRAGMENT = ROOT / "tools/orthogonal_event_budget_v5.swiftpart"
EVENT_LOGIC = ROOT / "tools/orthogonal_event_factor_v3_logic.swift"
BUDGET_LOGIC = ROOT / "tools/orthogonal_factor_family_v1_logic.swift"
ASSEMBLED = Path("/private/tmp/atm_orthogonal_event_budget_v5.swift")
BINARY = Path("/private/tmp/atm_orthogonal_event_budget_v5")
FORMAL_CANDIDATES = ["F-GROWTHBOND", "F-BIOTECH", "F-TRANSPORT"]
MATCHED_CONTROLS = {
    "F-GROWTHBOND": "C-GROWTHBOND-ALWAYS",
    "F-BIOTECH": "C-BIOTECH-ALWAYS",
    "F-TRANSPORT": "C-TRANSPORT-ALWAYS",
}
...
def compile_binary() -> None:
    ...
    run([
        ...
        str(BUDGET_LOGIC.relative_to(ROOT)),
        str(ASSEMBLED),
        "-o", str(BINARY),
    ], timeout=300)
...
def parse_candidate(block: str) -> dict:
    ...
    required = {
        ...
        "fingerprint", "source_events", "underinvested_events", "available_underinvested_events", "completed_events",
    ...
    return {
        ...
        "underinvested_events": int(values["underinvested_events"]),
        "available_underinvested_events": int(values["available_underinvested_events"]),
...
def validate_smoke(stdout: str) -> dict:
    ...
    underinvested_counts = [int(value) for value in re.findall(r"^SMOKE_EVENT_UNDERINVESTED_COUNT=(\d+)$", stdout, flags=re.MULTILINE)]
    ...
    expected_ids = [
        "C-GROWTHBOND-ALWAYS", "F-GROWTHBOND",
        "C-BIOTECH-ALWAYS", "F-BIOTECH",
        "C-TRANSPORT-ALWAYS", "F-TRANSPORT",
    ]
    ...
    if len(underinvested_counts) != 6 or len(set(underinvested_counts)) != 1 or underinvested_counts[0] <= 0:
        raise RuntimeError(f"smoke underinvested event count mismatch: {underinvested_counts}")
    ...
    return {
        ...
        "underinvested_events": underinvested_counts[0],
        "max_gross": gross,
        "min_weight": minimums,
    }
...
def write_outputs(output_dir: Path, rows: dict[str, dict]) -> dict:
    ...
    document = {
        ...
        "trial_id": "ATM-SVP2-ORTHO-FACTOR-005",
        "strategy_lineage": "orthogonal-event-budget-family-v5",
        "v11_control": v11,
    ...
    with (output_dir / "candidate-metrics.csv").open("w", encoding="utf-8", newline="") as handle:
        fields = [
            ...
            "source_events", "underinvested_events", "available_underinvested_events", "completed_events",
        ...
        writer.writerow({
            ...
            **{key: v11[key] for key in [
                ...
                "max_gross", "min_weight", "fingerprint", "source_events", "underinvested_events",
                "available_underinvested_events", "completed_events",
            ]},
...
def main() -> int:
    ...
    for name in ["QQQ.csv", "TLT.csv", "XBI.csv", "XLV.csv", "IYT.csv", "IEF.csv"]:
    ...
    env.update({
        ...
        "ATM_ORTHOGONAL_EVENT_BUDGET_V5": "1",
        "ATM_ORTHOGONAL_EVENT_BUDGET_V5_DATA_DIR": str(factor_dir),
    })
    output_dir = Path(args.output_dir)
    if args.formal:
        env["ATM_ORTHOGONAL_EVENT_BUDGET_V5_FORMAL"] = "1"
        env["ATM_ORTHOGONAL_EVENT_BUDGET_V5_OUTPUT_DIR"] = str(output_dir)
    stdout = run([str(BINARY)], env=env, timeout=300)
    if "ORTHOGONAL_EVENT_BUDGET_V5_COMPLETE" not in stdout:
    ...
    if not args.formal:
        ...
        print("ORTHOGONAL_EVENT_BUDGET_V5_SMOKE_OK")
        return 0
    ...
    print("ORTHOGONAL_EVENT_BUDGET_V5_FORMAL_COMPLETE")
    return 0
