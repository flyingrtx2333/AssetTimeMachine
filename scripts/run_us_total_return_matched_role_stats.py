#!/usr/bin/env python3
"""Frozen bootstrap + DSR audit for IWD versus matched SPY total-return role trial."""
...
def prior_sharpes() -> list[float]:
    ...
    for path in PRIOR_RESULT_PATHS:
        ...
        for result in document.get("candidate_results") or []:
            ...
            values.append(value)
    if len(values) != 40:
        raise RuntimeError(f"expected 40 prior performance-bearing candidates, got {len(values)}")
    return values
...
def main() -> int:
    ...
    if result["candidate_id"] != CANDIDATE_ID or result["matched_control_id"] != MATCHED_CONTROL_ID:
        raise RuntimeError("frozen IWD/SPY matched pair missing")
    ...
    if len(all_sharpes) != 41:
        raise RuntimeError("global DSR inventory must contain 41 candidates")
    ...
    document = {
        ...
        "method": {
            ...
            "price_field": "Yahoo adjusted_close_total_return_proxy for both IWD and SPY",
            "bootstrap_sampling": "paired circular moving blocks IWD versus SPY matched control",
            ...
            "global_post_protocol_dsr_prior_trial_count": 40,
            "global_post_protocol_dsr_total_trial_count": 41,
            "global_dsr_return_window": "2000-05-30+ candidate behavior only",
        },
        "candidate_results": [
            {
                "candidate_id": CANDIDATE_ID,
    ...
    print("US_TR_MATCHED_ROLE_V1_STATS_COMPLETE")
    return 0
