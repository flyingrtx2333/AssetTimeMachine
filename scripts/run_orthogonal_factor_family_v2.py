#!/usr/bin/env python3
"""Run the preregistered Orthogonal Factor Family V2 through the shared Swift engine."""
...
FRAGMENT = ROOT / "tools/orthogonal_factor_family_v2.swiftpart"
LOGIC = ROOT / "tools/orthogonal_factor_family_v2_logic.swift"
ASSEMBLED = Path("/private/tmp/atm_orthogonal_factor_family_v2.swift")
BINARY = Path("/private/tmp/atm_orthogonal_factor_family_v2")
FORMAL_CANDIDATES = ["F-FUNDING", "F-COPGOLD", "F-SKEW"]
...
def validate_smoke(stdout: str) -> dict:
    ...
    if len(factor_counts) != 5 or any(value <= 0 for value in factor_counts):
...
def write_outputs(output_dir: Path, rows: dict[str, dict]) -> dict:
    ...
    document = {
        ...
        "trial_id": "ATM-SVP2-ORTHO-FACTOR-002",
        "strategy_lineage": "orthogonal-factor-family-v2",
...
def main() -> int:
    ...
    for name in ["DCPF3M.csv", "DFF.csv", "HG.csv", "GC.csv", "SKEW.csv"]:
    ...
    env.update({
        ...
        "ATM_ORTHOGONAL_FACTOR_V2": "1",
        "ATM_ORTHOGONAL_FACTOR_V2_DATA_DIR": str(factor_dir),
    })
    output_dir = Path(args.output_dir)
    if args.formal:
        env["ATM_ORTHOGONAL_FACTOR_V2_FORMAL"] = "1"
        env["ATM_ORTHOGONAL_FACTOR_V2_OUTPUT_DIR"] = str(output_dir)
    stdout = run([str(BINARY)], env=env, timeout=300)
    if "ORTHOGONAL_FACTOR_FAMILY_V2_COMPLETE" not in stdout:
    ...
    if not args.formal:
        ...
        print("ORTHOGONAL_FACTOR_V2_SMOKE_OK")
        return 0
    ...
    print("ORTHOGONAL_FACTOR_V2_FORMAL_COMPLETE")
    return 0
