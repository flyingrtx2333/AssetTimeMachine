...
FRAGMENT = ROOT / "tools/v11_c3l3_adaptive_core_v1.swiftpart"
ASSEMBLED = Path("/private/tmp/atm_v11_c3l3_adaptive_core_v1.swift")
BINARY = Path("/private/tmp/atm_v11_c3l3_adaptive_core_v1")
CANDIDATES = ("S-V11-C3L3-CORE-SWITCH",)
...
def event_audit(stdout: str) -> dict[str, int]:
    patterns = {
        "source_events": r"^source_event_count=(\d+)$",
        "stress_events": r"^stress_event_count=(\d+)$",
        "calm_events": r"^calm_event_count=(\d+)$",
        "low_fee_stress_events": r"^low_fee_stress_event_count=(\d+)$",
    }
    values: dict[str, int] = {}
    for key, pattern in patterns.items():
        match = re.search(pattern, stdout, flags=re.MULTILINE)
        if not match:
            raise RuntimeError(f"missing event audit field: {key}")
        values[key] = int(match.group(1))
    if values["source_events"] != values["stress_events"] + values["calm_events"]:
        raise RuntimeError("stress/calm event accounting mismatch")
    if values["stress_events"] != values["low_fee_stress_events"]:
        raise RuntimeError("frozen C3/L3 state changes with user fee")
    return values


def validate_smoke(stdout: str) -> dict:
    control_fp, candidate_fp, low_fee_fp = extract_fingerprints(stdout)
    events = event_audit(stdout)
    ...
    return {"control_fingerprint": control_fp, "candidate_fingerprint": candidate_fp, "fee_invariant_target": True, "max_gross": max_gross, "min_weight": min_weight, **events}
...
def write_outputs(output_dir: Path, rows: dict[str, dict], stdout: str) -> dict:
    ...
    document = {
        "protocol_id": "ATM-SVP-2", "trial_id": "ATM-SVP2-V11-C3L3-CORE-SWITCH-001", "strategy_lineage": "v11-c3l3-adaptive-core-v1",
...
def main() -> int:
    ...
    compile_binary(); env=os.environ.copy(); env.update({"ATM_HISTORY_FIXTURE":str(fixture),"ATM_V11_C3L3_ADAPTIVE_CORE_V1":"1"}); output=Path(args.output_dir)
    if args.formal: env["ATM_V11_C3L3_ADAPTIVE_CORE_V1_FORMAL"]="1"; env["ATM_V11_C3L3_ADAPTIVE_CORE_V1_OUTPUT_DIR"]=str(output)
    stdout=run([str(BINARY)],env=env,timeout=300)
    if "V11_C3L3_ADAPTIVE_CORE_V1_COMPLETE" not in stdout: raise RuntimeError("Swift high-core trial did not complete")
    if not args.formal:
        smoke=validate_smoke(stdout); print(json.dumps({"mode":"SMOKE_NO_PERFORMANCE",**smoke},ensure_ascii=False,sort_keys=True)); print("V11_C3L3_ADAPTIVE_CORE_V1_SMOKE_OK"); return 0
    document=write_outputs(output,parse_formal(stdout),stdout); print(json.dumps(document,ensure_ascii=False,sort_keys=True)); print("V11_C3L3_ADAPTIVE_CORE_V1_FORMAL_COMPLETE"); return 0
