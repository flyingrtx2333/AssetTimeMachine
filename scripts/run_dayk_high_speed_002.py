#!/usr/bin/env python3
"""Run the preregistered 日K高速002 trial through the shared Swift research engine."""
...
FRAGMENT = ROOT / "tools/dayk_high_speed_002.swiftpart"
LOGIC = ROOT / "tools/dayk_high_speed_002_logic.swift"
ASSEMBLED = Path("/private/tmp/atm_dayk_high_speed_002.swift")
BINARY = Path("/private/tmp/atm_dayk_high_speed_002")
...
def validate_document(document: dict) -> None:
    ...
    expected_candidates = {"DKHS002-GOLD", "DKHS002-NASDAQ", "DKHS002-SP500", "DKHS002-COMBINED"}
    expected_controls = {"C-COMPRESSION-GOLD", "C-COMPRESSION-NASDAQ", "C-COMPRESSION-SP500", "C-COMPRESSION-COMBINED"}
    ...
    if document.get("evaluation_start") != "2001-09-05" or document.get("evaluation_end") != "2026-08-20":
...
def evaluate(document: dict) -> dict:
    combined = path_by_id(document, "candidate_paths", "DKHS002-COMBINED")
    matched = path_by_id(document, "matched_control_paths", "C-COMPRESSION-COMBINED")
    cash = document["cash_control"]
    stress = document["slippage_stress"]
    standalone = [
        path_by_id(document, "candidate_paths", "DKHS002-GOLD"),
        path_by_id(document, "candidate_paths", "DKHS002-NASDAQ"),
        path_by_id(document, "candidate_paths", "DKHS002-SP500"),
    ]
    fold_sharpes = [float(row["sharpe"]) for row in combined["folds"]]
    if len(fold_sharpes) != 7:
        raise RuntimeError("combined path must contain seven fixed folds")
    ...
    invariant_checks = {
        "max_intraday_gross_le_100pct": float(combined["max_intraday_gross"]) <= 1.000000001,
        "minimum_end_of_day_cash_nonnegative": float(combined["minimum_end_of_day_cash"]) >= -1e-8,
        "zero_overnight_position_days": int(combined["overnight_position_days"]) == 0,
    }
    checks = {
        "at_least_two_assets_outperform_cash": profitable_assets >= 2,
        "at_least_two_assets_positive_sharpe": positive_sharpe_assets >= 2,
        "each_asset_at_least_150_round_trips": minimum_asset_trades >= 150,
        "combined_cagr_ge_cash_plus_2pp": float(combined["cagr_percent"]) >= cash_cagr + 2.0,
        "combined_sharpe_ge_1": float(combined["sharpe"]) >= 1.0,
        "combined_mdd_le_10pct": float(combined["mdd_percent"]) <= 10.0,
        "combined_sharpe_gt_matched_compression_control": float(combined["sharpe"]) > float(matched["sharpe"]),
        "combined_average_trade_return_positive": float(trade_stats["average_return_percent"]) > 0,
        "combined_profit_factor_ge_1_05": float(trade_stats["profit_factor"]) >= 1.05,
        "at_least_five_of_seven_positive_sharpe_folds": positive_folds >= 5,
...
def parse_formal_stdout(stdout: str) -> dict:
    prefix = "DAYK_HIGH_SPEED_002_FORMAL_JSON="
    lines = [line[len(prefix):] for line in stdout.splitlines() if line.startswith(prefix)]
    if len(lines) != 1 or "DAYK_HIGH_SPEED_002_FORMAL_OK" not in stdout:
...
def main() -> int:
    ...
    env.update({
        ...
        "ATM_DAYK_HIGH_SPEED_002": "1",
    })
    if args.formal:
        env["ATM_DAYK_HIGH_SPEED_002_FORMAL"] = "1"
        env["ATM_DAYK_HIGH_SPEED_002_OUTPUT_DIR"] = str(Path(args.output_dir))
    stdout = run([str(BINARY)], env=env)
    if not args.formal:
        if "DAYK_HIGH_SPEED_002_SMOKE_OK" not in stdout:
    ...
    print("DAYK_HIGH_SPEED_002_FORMAL_COMPLETE")
    return 0
