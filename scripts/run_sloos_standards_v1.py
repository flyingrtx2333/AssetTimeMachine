#!/usr/bin/env python3
"""Run preregistered SLOOS Standards V1 through the shared Swift App engine."""
...
FRAGMENT = ROOT / "tools/sloos_standards_v1.swiftpart"
SLOOS_LOGIC = ROOT / "tools/sloos_standards_v1_logic.swift"
EVENT_LOGIC = ROOT / "tools/orthogonal_event_factor_v3_logic.swift"
ASSEMBLED = Path("/private/tmp/atm_sloos_standards_v1.swift")
BINARY = Path("/private/tmp/atm_sloos_standards_v1")
CANDIDATE_ID = "F-SLOOS-STANDARDS"
MATCHED_CONTROL_ID = "C-SLOOS-ALWAYS"
...
def compile_binary() -> None:
    ...
    run([
        ...
        str(EVENT_LOGIC.relative_to(ROOT)),
        str(SLOOS_LOGIC.relative_to(ROOT)),
        str(ASSEMBLED),
        "-o", str(BINARY),
    ], timeout=300)
...
def evaluate_candidate(metrics: dict, *, v11: dict, matched_control: dict) -> dict:
    ...
    constraints_pass = float(metrics["max_gross"]) <= 1.000000001 and float(metrics["min_weight"]) >= -1e-10
    checks = {
        ...
        "folds_sharpe_ge_matched_ge_4_of_6": fold_wins >= 4,
...
def validate_smoke(stdout: str) -> dict:
    ...
    ids = re.findall(r"^SMOKE_SLOOS_PATH=(.+)$", stdout, flags=re.MULTILINE)
    source_counts = [int(value) for value in re.findall(r"^SMOKE_SLOOS_SOURCE_COUNT=(\d+)$", stdout, flags=re.MULTILINE)]
    derisk_counts = [int(value) for value in re.findall(r"^SMOKE_SLOOS_DERISK_COUNT=(\d+)$", stdout, flags=re.MULTILINE)]
    available_counts = [int(value) for value in re.findall(r"^SMOKE_SLOOS_AVAILABLE_COUNT=(\d+)$", stdout, flags=re.MULTILINE)]
    ...
    if len(derisk_counts) != 2 or len(set(derisk_counts)) != 1 or derisk_counts[0] <= 0:
        raise RuntimeError(f"de-risk count mismatch: {derisk_counts}")
    ...
    return {
        ...
        "available_derisk_events": available_counts[0],
...
def write_outputs(output_dir: Path, rows: dict[str, dict]) -> dict:
    ...
    document = {
        ...
        "trial_id": "ATM-SVP2-SLOOS-001",
        "strategy_lineage": "sloos-standards-v1",
        "evaluation_window_start": "2010-05-01",
        "v11_control": v11,
    ...
    fields = [
        ...
        "average_cash_ratio", "max_gross", "min_weight", "source_events", "derisk_events",
        "available_derisk_events", "retained_events", "folds_sharpe_ge_matched", "worst_fold_sharpe",
        ...
        for path_id, kind in [("V11-CONTROL", "CONTROL"), (MATCHED_CONTROL_ID, "MATCHED_CONTROL")]:
            ...
            writer.writerow({
                ...
                **{key: path[key] for key in [
                    ...
                    "derisk_events", "available_derisk_events", "retained_events",
                ]},
...
def main() -> int:
    ...
    env.update({
        ...
        "ATM_SLOOS_STANDARDS_V1": "1",
        "ATM_SLOOS_STANDARDS_V1_FACTOR_PATH": str(factor_input),
    })
    output_dir = Path(args.output_dir)
    if args.formal:
        env["ATM_SLOOS_STANDARDS_V1_FORMAL"] = "1"
        env["ATM_SLOOS_STANDARDS_V1_OUTPUT_DIR"] = str(output_dir)
    stdout = run([str(BINARY)], env=env, timeout=300)
    if "SLOOS_STANDARDS_V1_COMPLETE" not in stdout:
        raise RuntimeError("Swift SLOOS trial did not complete")
    ...
    if not args.formal:
        ...
        print("SLOOS_STANDARDS_V1_SMOKE_OK")
        return 0
    ...
    print("SLOOS_STANDARDS_V1_FORMAL_COMPLETE")
    return 0
