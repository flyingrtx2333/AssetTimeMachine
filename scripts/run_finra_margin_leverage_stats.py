#!/usr/bin/env python3
"""Frozen bootstrap and cumulative DSR audit for ATM-SVP2-MARGIN-001."""
...
def prior_post_protocol_sharpes() -> list[float]:
    ...
    if len(sharpes) != 26:
        raise RuntimeError(f"global DSR prior inventory must contain 26 candidates, got {len(sharpes)}")
    return sharpes


def global_dsr(candidate_dir: Path, candidate_sharpe: float) -> dict:
    trial_sharpes = prior_post_protocol_sharpes() + [candidate_sharpe]
    if len(trial_sharpes) != 27:
        raise RuntimeError("global DSR inventory must contain 27 candidates")
...
def main() -> int:
    ...
    if result["candidate_id"] != CANDIDATE_ID:
        raise RuntimeError("candidate metrics does not contain frozen FINRA margin candidate")
    ...
    document = {
        ...
        "method": {
            ...
            "global_post_protocol_dsr_trial_count": 27,
        },
        "candidate_results": [{
            "candidate_id": CANDIDATE_ID,
    ...
    print("FINRA_MARGIN_LEVERAGE_V1_STATS_COMPLETE")
    return 0
