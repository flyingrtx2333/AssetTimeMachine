#!/usr/bin/env python3
"""Run the preregistered annual cross-asset winner strategy."""
...
FRAGMENT = ROOT / "tools/annual_winner_001.swiftpart"
LOGIC = ROOT / "tools/annual_winner_001_logic.swift"
ASSEMBLED = Path("/private/tmp/atm_annual_winner_001.swift")
BINARY = Path("/private/tmp/atm_annual_winner_001")
...
def validate_document(document: dict) -> None:
    ...
    if document.get("evaluation_start") != "2003-01-01" or document.get("evaluation_end") != "2026-08-20":
...
def parse_formal_stdout(stdout: str) -> dict:
    prefix = "ANNUAL_WINNER_001_FORMAL_JSON="
    lines = [line[len(prefix):] for line in stdout.splitlines() if line.startswith(prefix)]
    if len(lines) != 1 or "ANNUAL_WINNER_001_FORMAL_OK" not in stdout:
...
def main() -> int:
    ...
    compile_binary()
    env = os.environ.copy()
    env.update({"ATM_HISTORY_FIXTURE": str(fixture), "ATM_ANNUAL_WINNER_001": "1"})
    if args.formal:
        env["ATM_ANNUAL_WINNER_001_FORMAL"] = "1"
        env["ATM_ANNUAL_WINNER_001_OUTPUT_DIR"] = str(Path(args.output_dir))
    stdout = run([str(BINARY)], env=env)
    if not args.formal:
        if "ANNUAL_WINNER_001_SMOKE_OK" not in stdout:
            print(stdout, end="")
    ...
    print("ANNUAL_WINNER_001_FORMAL_COMPLETE")
    return 0
