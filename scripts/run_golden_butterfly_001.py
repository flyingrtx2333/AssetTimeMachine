...
TRIAL_ID = "ATM-SVP3-GOLDEN-BUTTERFLY-001"
CANDIDATE_ID = "S-GOLDEN-BUTTERFLY-001"
FRAGMENT = ROOT / "tools/golden_butterfly_001.swiftpart"
LOGIC = ROOT / "tools/golden_butterfly_001_logic.swift"
ASSEMBLED = Path("/private/tmp/atm_golden_butterfly_001.swift")
BINARY = Path("/private/tmp/atm_golden_butterfly_001")
...
def parse_formal_stdout(stdout: str) -> dict:
    prefix = "GOLDEN_BUTTERFLY_001_FORMAL_JSON="
    lines = [line[len(prefix):] for line in stdout.splitlines() if line.startswith(prefix)]
    if len(lines) != 1 or "GOLDEN_BUTTERFLY_001_FORMAL_OK" not in stdout:
...
def main() -> int:
    ...
    compile_binary()
    env = os.environ.copy()
    env.update({"ATM_HISTORY_FIXTURE": str(fixture), "ATM_GOLDEN_BUTTERFLY_001": "1"})
    if args.formal:
        env["ATM_GOLDEN_BUTTERFLY_001_FORMAL"] = "1"
        env["ATM_GOLDEN_BUTTERFLY_001_OUTPUT_DIR"] = str(Path(args.output_dir))
    stdout = run([str(BINARY)], env=env)
    if not args.formal:
        if "GOLDEN_BUTTERFLY_001_SMOKE_OK" not in stdout:
            print(stdout, end="")
    ...
    print("GOLDEN_BUTTERFLY_001_FORMAL_COMPLETE")
    return 0
