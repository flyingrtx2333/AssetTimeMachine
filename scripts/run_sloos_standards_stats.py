#!/usr/bin/env python3
"""Frozen bootstrap and cumulative DSR audit for ATM-SVP2-SLOOS-001."""
...
def aligned_returns(paths: dict[str, Path], *, start_date: str | None = None) -> tuple[list[str], dict[str, np.ndarray]]:
...
def prior_post_protocol_sharpes() -> list[float]:
    ...
    if len(sharpes) != 32:
        raise RuntimeError(f"global DSR prior inventory must contain 32 candidates, got {len(sharpes)}")
    return sharpes


def global_dsr(candidate_dir: Path, candidate_sharpe: float) -> dict:
    trial_sharpes = prior_post_protocol_sharpes() + [candidate_sharpe]
    if len(trial_sharpes) != 33:
        raise RuntimeError("global DSR inventory must contain 33 candidates")
...
def main() -> int:
    ...
    if result["candidate_id"] != CANDIDATE_ID:
        raise RuntimeError("candidate metrics does not contain frozen SLOOS candidate")
    ...
    document = {
        ...
        "method": {
            ...
            "global_dsr_sample": "full-history candidate returns for compatibility with prior post-protocol DSR accounting",
            "global_post_protocol_dsr_trial_count": 33,
        },
        "candidate_results": [{
            "candidate_id": CANDIDATE_ID,
    ...
    print("SLOOS_STANDARDS_V1_STATS_COMPLETE")
    return 0
