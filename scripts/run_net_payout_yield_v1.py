#!/usr/bin/env python3
"""Run preregistered ATM-SVP2-NPY-001 through the shared Swift engine."""
...
FRAGMENT = ROOT / "tools/net_payout_yield_v1.swiftpart"
LOGIC = ROOT / "tools/net_payout_yield_v1_logic.swift"
ASSEMBLED = Path("/private/tmp/atm_net_payout_yield_v1.swift")
BINARY = Path("/private/tmp/atm_net_payout_yield_v1")
CANDIDATE_ID = "F-NET-PAYOUT-YIELD-US"
MATCHED_CONTROL_ID = "C-NET-PAYOUT-ALWAYS"
...
def validate_smoke(stdout: str) -> dict:
    ...
    ids = re.findall(r"^SMOKE_NPY_PATH=(.+)$", stdout, flags=re.MULTILINE)
    source_counts = [int(value) for value in re.findall(r"^SMOKE_NPY_SOURCE_COUNT=(\d+)$", stdout, flags=re.MULTILINE)]
    derisk_counts = [int(value) for value in re.findall(r"^SMOKE_NPY_US_DERISK_COUNT=(\d+)$", stdout, flags=re.MULTILINE)]
    available_counts = [int(value) for value in re.findall(r"^SMOKE_NPY_AVAILABLE_COUNT=(\d+)$", stdout, flags=re.MULTILINE)]
...
def write_outputs(output_dir: Path, rows: dict[str, dict]) -> dict:
    ...
    document = {
        ...
        "trial_id": "ATM-SVP2-NPY-001",
        "strategy_lineage": "net-payout-yield-v1",
        "v11_control": v11,
...
def main() -> int:
    ...
    env.update({
        ...
        "ATM_NET_PAYOUT_YIELD_V1": "1",
        "ATM_NET_PAYOUT_YIELD_V1_FACTOR_PATH": str(factor_input),
    })
    output_dir = Path(args.output_dir)
    if args.formal:
        env["ATM_NET_PAYOUT_YIELD_V1_FORMAL"] = "1"
        env["ATM_NET_PAYOUT_YIELD_V1_OUTPUT_DIR"] = str(output_dir)
    stdout = run([str(BINARY)], env=env, timeout=300)
    if "NET_PAYOUT_YIELD_V1_COMPLETE" not in stdout:
        raise RuntimeError("Swift net payout yield factor trial did not complete")
    ...
    if not args.formal:
        ...
        print("NET_PAYOUT_YIELD_V1_SMOKE_OK")
        return 0
    ...
    print("NET_PAYOUT_YIELD_V1_FORMAL_COMPLETE")
    return 0
