#!/usr/bin/env python3
"""Run preregistered ATM-SVP2-MARGIN-001 through the shared Swift engine."""
...
FRAGMENT = ROOT / "tools/finra_margin_leverage_v1.swiftpart"
LOGIC = ROOT / "tools/finra_margin_leverage_v1_logic.swift"
ASSEMBLED = Path("/private/tmp/atm_finra_margin_leverage_v1.swift")
BINARY = Path("/private/tmp/atm_finra_margin_leverage_v1")
CANDIDATE_ID = "F-MARGIN-LEV-US"
MATCHED_CONTROL_ID = "C-MARGIN-LEV-ALWAYS"
...
def validate_smoke(stdout: str) -> dict:
    ...
    ids = re.findall(r"^SMOKE_MARGIN_PATH=(.+)$", stdout, flags=re.MULTILINE)
    source_counts = [int(value) for value in re.findall(r"^SMOKE_MARGIN_SOURCE_COUNT=(\d+)$", stdout, flags=re.MULTILINE)]
    derisk_counts = [int(value) for value in re.findall(r"^SMOKE_MARGIN_US_DERISK_COUNT=(\d+)$", stdout, flags=re.MULTILINE)]
    available_counts = [int(value) for value in re.findall(r"^SMOKE_MARGIN_AVAILABLE_COUNT=(\d+)$", stdout, flags=re.MULTILINE)]
...
def main() -> int:
    ...
    env.update({
        ...
        "ATM_FINRA_MARGIN_LEVERAGE_V1": "1",
        "ATM_FINRA_MARGIN_LEVERAGE_V1_FACTOR_PATH": str(factor_input),
    })
    output_dir = Path(args.output_dir)
    if args.formal:
        env["ATM_FINRA_MARGIN_LEVERAGE_V1_FORMAL"] = "1"
        env["ATM_FINRA_MARGIN_LEVERAGE_V1_OUTPUT_DIR"] = str(output_dir)
    stdout = run([str(BINARY)], env=env, timeout=300)
    if "FINRA_MARGIN_LEVERAGE_V1_COMPLETE" not in stdout:
        raise RuntimeError("Swift FINRA margin factor trial did not complete")
    ...
    if not args.formal:
        ...
        print("FINRA_MARGIN_LEVERAGE_V1_SMOKE_OK")
        return 0
    ...
    print("FINRA_MARGIN_LEVERAGE_V1_FORMAL_COMPLETE")
    return 0
