#!/usr/bin/env python3
"""Frozen bootstrap + cumulative DSR audit for the production-path value-role trial for ATM-SVP2-US-VALUE-PROD-001."""
...
def prior_sharpes() -> list[float]:
    ...
    for path in PRIOR_RESULT_PATHS:
        ...
        for result in document.get("candidate_results") or []:
            ...
            values.append(value)
    if len(values)!=38: raise RuntimeError(f"expected 38 prior candidates, got {len(values)}")
    return values
...
def main() -> int:
    ...
    if len(all_sharpes)!=40: raise RuntimeError("global DSR inventory must contain 40 candidates")
    results=[]
