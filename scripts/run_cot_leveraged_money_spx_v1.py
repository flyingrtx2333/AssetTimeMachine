#!/usr/bin/env python3
"""Run preregistered ATM-SVP2-COT-001 through the shared Swift engine."""
...
FRAGMENT = ROOT / "tools/cot_leveraged_money_spx_v1.swiftpart"
LOGIC = ROOT / "tools/cot_leveraged_money_spx_v1_logic.swift"
ASSEMBLED = Path("/private/tmp/atm_cot_leveraged_money_spx_v1.swift")
BINARY = Path("/private/tmp/atm_cot_leveraged_money_spx_v1")
...
def validate_smoke(stdout: str) -> dict:
    ...
    ids = re.findall(r"^SMOKE_COT_PATH=(.+)$", stdout, flags=re.MULTILINE)
    source_counts = [int(value) for value in re.findall(r"^SMOKE_COT_SOURCE_COUNT=(\d+)$", stdout, flags=re.MULTILINE)]
    derisk_counts = [int(value) for value in re.findall(r"^SMOKE_COT_US_DERISK_COUNT=(\d+)$", stdout, flags=re.MULTILINE)]
    available_counts = [int(value) for value in re.findall(r"^SMOKE_COT_AVAILABLE_COUNT=(\d+)$", stdout, flags=re.MULTILINE)]
...
def write_outputs(output_dir: Path, rows: dict[str, dict]) -> dict:
    ...
    document = {
        ...
        "strategy_lineage": "cot-leveraged-money-spx-v1",
        "v11_control": v11,
...
def main() -> int:
    ...
    env.update({
        ...
        "ATM_COT_LEVERAGED_MONEY_SPX_V1": "1",
        "ATM_COT_LEVERAGED_MONEY_SPX_V1_FACTOR_PATH": str(factor_input),
    })
    output_dir = Path(args.output_dir)
    if args.formal:
        env["ATM_COT_LEVERAGED_MONEY_SPX_V1_FORMAL"] = "1"
        env["ATM_COT_LEVERAGED_MONEY_SPX_V1_OUTPUT_DIR"] = str(output_dir)
    stdout = run([str(BINARY)], env=env, timeout=300)
    if "COT_LEVERAGED_MONEY_SPX_V1_COMPLETE" not in stdout:
        raise RuntimeError("Swift COT factor trial did not complete")
    ...
    if not args.formal:
        ...
        print("COT_LEVERAGED_MONEY_SPX_V1_SMOKE_OK")
        return 0
    ...
    print("COT_LEVERAGED_MONEY_SPX_V1_FORMAL_COMPLETE")
    return 0
