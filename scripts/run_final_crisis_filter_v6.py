#!/usr/bin/env python3
"""Run preregistered Final Crisis Filter Family V6 through the shared Swift engine."""
...
FRAGMENT = ROOT / "tools/final_crisis_filter_v6.swiftpart"
...
CRISIS_LOGIC = ROOT / "tools/final_crisis_filter_v6_logic.swift"
ASSEMBLED = Path("/private/tmp/atm_final_crisis_filter_v6.swift")
BINARY = Path("/private/tmp/atm_final_crisis_filter_v6")
FORMAL_CANDIDATES = ["F-VIXTERM", "F-VVIX", "F-CREDITCASH"]
MATCHED_CONTROLS = {
...
def compile_binary() -> None:
    ...
    run([
        ...
        str(CRISIS_LOGIC.relative_to(ROOT)),
        str(ASSEMBLED),
        "-o", str(BINARY),
    ], timeout=300)
...
def validate_smoke(stdout: str) -> dict:
    ...
    expected_ids = [
        "C-VIXTERM-ALWAYS", "F-VIXTERM",
        "C-VVIX-ALWAYS", "F-VVIX",
        "C-CREDITCASH-ALWAYS", "F-CREDITCASH",
    ]
...
def write_outputs(output_dir: Path, rows: dict[str, dict]) -> dict:
    ...
    document = {
        ...
        "strategy_lineage": "final-crisis-filter-event-budget-v6",
        "v11_control": v11,
...
def main() -> int:
    ...
    for name in ["VIX9D.csv", "VIX.csv", "VVIX.csv", "HYG.csv", "SHY.csv"]:
    ...
    env.update({
        ...
        "ATM_FINAL_CRISIS_FILTER_V6": "1",
        "ATM_FINAL_CRISIS_FILTER_V6_DATA_DIR": str(factor_dir),
    })
    output_dir = Path(args.output_dir)
    if args.formal:
        env["ATM_FINAL_CRISIS_FILTER_V6_FORMAL"] = "1"
        env["ATM_FINAL_CRISIS_FILTER_V6_OUTPUT_DIR"] = str(output_dir)
    stdout = run([str(BINARY)], env=env, timeout=300)
    if "FINAL_CRISIS_FILTER_V6_COMPLETE" not in stdout:
    ...
    if not args.formal:
        ...
        print("FINAL_CRISIS_FILTER_V6_SMOKE_OK")
        return 0
    ...
    print("FINAL_CRISIS_FILTER_V6_FORMAL_COMPLETE")
    return 0
