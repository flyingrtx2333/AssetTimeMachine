#!/usr/bin/env python3
"""Run the preregistered 日K高速006 trial through the shared Swift research engine."""
...
TRIAL_ID = "ATM-SVP2-DAYK-HS-006"
CANDIDATE_ID = "S-DAYK-HIGH-SPEED-006"
FRAGMENT = ROOT / "tools/dayk_high_speed_006.swiftpart"
LOGIC = ROOT / "tools/dayk_high_speed_006_logic.swift"
ASSEMBLED = Path("/private/tmp/atm_dayk_high_speed_006.swift")
BINARY = Path("/private/tmp/atm_dayk_high_speed_006")
...
def validate_document(document: dict) -> None:
    ...
    expected_candidates = {"DKHS006-GOLD", "DKHS006-NASDAQ", "DKHS006-SP500", "DKHS006-COMBINED"}
    expected_controls = {"C-CONSECUTIVE-NO-SMA-GOLD", "C-CONSECUTIVE-NO-SMA-NASDAQ", "C-CONSECUTIVE-NO-SMA-SP500", "C-CONSECUTIVE-NO-SMA-COMBINED"}
...
def evaluate(document: dict) -> dict:
    combined = path_by_id(document, "candidate_paths", "DKHS006-COMBINED")
    matched = path_by_id(document, "matched_control_paths", "C-CONSECUTIVE-NO-SMA-COMBINED")
    cash = document["cash_control"]
    stress = document["slippage_stress"]
    standalone = [
        path_by_id(document, "candidate_paths", "DKHS006-GOLD"),
        path_by_id(document, "candidate_paths", "DKHS006-NASDAQ"),
        path_by_id(document, "candidate_paths", "DKHS006-SP500"),
    ]
    ...
    invariant_checks = {
        ...
        "zero_time_forced_exits": sum(int(x) for x in combined["time_exits_by_symbol"].values()) == 0,
        "all_completed_trades_are_stop_exits": sum(int(x) for x in combined["stop_exits_by_symbol"].values()) == int(trade_stats["round_trips"]),
        "final_open_positions_le_asset_count": int(combined["final_open_positions"]) <= 3,
    }
    checks = {
        "at_least_two_assets_outperform_cash": profitable_assets >= 2,
        "at_least_two_assets_positive_sharpe": positive_sharpe_assets >= 2,
        "each_asset_at_least_65_round_trips": minimum_asset_trades >= 65,
        "combined_cagr_ge_cash_plus_2pp": float(combined["cagr_percent"]) >= cash_cagr + 2.0,
        "combined_sharpe_ge_1": float(combined["sharpe"]) >= 1.0,
        "combined_mdd_le_20pct": float(combined["mdd_percent"]) <= 20.0,
        "combined_sharpe_gt_matched_no_sma_control": float(combined["sharpe"]) > float(matched["sharpe"]),
...
def parse_formal_stdout(stdout: str) -> dict:
    prefix = "DAYK_HIGH_SPEED_006_FORMAL_JSON="
    lines = [line[len(prefix):] for line in stdout.splitlines() if line.startswith(prefix)]
    if len(lines) != 1 or "DAYK_HIGH_SPEED_006_FORMAL_OK" not in stdout:
...
def main() -> int:
    ...
    env.update({
        ...
        "ATM_DAYK_HIGH_SPEED_006": "1",
    })
    if args.formal:
        env["ATM_DAYK_HIGH_SPEED_006_FORMAL"] = "1"
        env["ATM_DAYK_HIGH_SPEED_006_OUTPUT_DIR"] = str(Path(args.output_dir))
    stdout = run([str(BINARY)], env=env)
    if not args.formal:
        if "DAYK_HIGH_SPEED_006_SMOKE_OK" not in stdout:
    ...
    print("DAYK_HIGH_SPEED_006_FORMAL_COMPLETE")
    return 0
