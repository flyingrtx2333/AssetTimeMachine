...
ROOT=Path(__file__).resolve().parents[1]; TRIAL_ID='ATM-SVP3-VIX-PANIC-001'; CANDIDATE_ID='S-VIX-PANIC-001'; FRAGMENT=ROOT/'tools/vix_panic_001.swiftpart'; LOGIC=ROOT/'tools/vrp_proxy_v1_logic.swift'; EXTRA_LOGIC=ROOT/'tools/vix_panic_001_logic.swift'; ASSEMBLED=Path('/private/tmp/atm_vix_panic_001.swift'); BINARY=Path('/private/tmp/atm_vix_panic_001'); VIX=ROOT/'tools/research-results/strategy-validation/factor-data/ATM-SVP2-ORTHO-FACTOR-006/VIX.csv'
...
def compile_binary():
 ...
 run(['xcrun','swiftc','-parse-as-library','-module-cache-path','/private/tmp/atm-swift-module-cache','AssetTimeMachine/Backtest/BacktestModels.swift','AssetTimeMachine/Backtest/BacktestMetricsCalculator.swift','AssetTimeMachine/Backtest/BacktestSeriesAlignment.swift','AssetTimeMachine/Backtest/BacktestFXConverter.swift','AssetTimeMachine/Backtest/BacktestAdvancedSeriesPreparer.swift','AssetTimeMachine/Backtest/BacktestEngine.swift',str(LOGIC.relative_to(ROOT)),str(EXTRA_LOGIC.relative_to(ROOT)),str(ASSEMBLED),'-o',str(BINARY)])
def parse(stdout):
 prefix='VIX_PANIC_001_FORMAL_JSON='; rows=[x[len(prefix):] for x in stdout.splitlines() if x.startswith(prefix)]
 if len(rows)!=1 or 'VIX_PANIC_001_FORMAL_OK' not in stdout: raise RuntimeError('formal output incomplete')
 d=json.loads(rows[0]); assert d['trial_id']==TRIAL_ID and d['candidate_id']==CANDIDATE_ID and abs(d['vix_threshold']-30.0)<1e-12 and d['max_stale_calendar_days']==7 and d['evaluation_start']=='2002-07-03' and abs(d['fee_percent_per_trade']-1)<1e-12 and abs(d['slippage_percent_per_trade']-.05)<1e-12; return d
def evaluate(d):
...
def main():
 ap=argparse.ArgumentParser(); ap.add_argument('--fixture',required=True); ap.add_argument('--output-dir'); ap.add_argument('--formal',action='store_true'); a=ap.parse_args(); compile_binary(); env=os.environ.copy(); env.update({'ATM_HISTORY_FIXTURE':a.fixture,'ATM_VIX_PANIC_001':'1','ATM_VIX_PANIC_001_VIX':str(VIX)}); 
 if a.formal: env.update({'ATM_VIX_PANIC_001_FORMAL':'1','ATM_VIX_PANIC_001_OUTPUT_DIR':a.output_dir})
 stdout=run([str(BINARY)],env=env)
 if not a.formal:
  if 'VIX_PANIC_001_SMOKE_OK' not in stdout: print(stdout,end=''); raise RuntimeError('smoke failed')
  print(stdout,end=''); return 0
 d=parse(stdout); e=evaluate(d); write_outputs(Path(a.output_dir),d,e); print(json.dumps({'trial_id':TRIAL_ID,'candidate_id':CANDIDATE_ID,**e},ensure_ascii=False,sort_keys=True)); print('VIX_PANIC_001_FORMAL_COMPLETE'); return 0
