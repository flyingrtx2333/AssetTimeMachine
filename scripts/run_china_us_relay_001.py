...
FRAGMENT = ROOT / "tools/china_us_relay_001.swiftpart"
LOGIC = ROOT / "tools/china_us_relay_001_logic.swift"
ASSEMBLED = Path("/private/tmp/atm_china_us_relay_001.swift")
BINARY = Path("/private/tmp/atm_china_us_relay_001")
...
def validate_document(document: dict) -> None:
    ...
    if candidate_ids != {"CUS001-NASDAQ", "CUS001-SP500", "CUS001-COMBINED"}:
...
def evaluate(document: dict) -> dict:
    combined = path_by_id(document, "candidate_paths", "CUS001-COMBINED")
    nasdaq = path_by_id(document, "candidate_paths", "CUS001-NASDAQ")
    sp500 = path_by_id(document, "candidate_paths", "CUS001-SP500")
...
def parse_formal_stdout(stdout: str) -> dict:
    prefix = "CHINA_US_RELAY_001_FORMAL_JSON="
    lines = [line[len(prefix):] for line in stdout.splitlines() if line.startswith(prefix)]
    if len(lines) != 1 or "CHINA_US_RELAY_001_FORMAL_OK" not in stdout:
...
def main() -> int:
    ...
    compile_binary()
    env = os.environ.copy()
    env.update({"ATM_HISTORY_FIXTURE": str(fixture), "ATM_CHINA_US_RELAY_001": "1"})
    if args.formal:
        env["ATM_CHINA_US_RELAY_001_FORMAL"] = "1"
        env["ATM_CHINA_US_RELAY_001_OUTPUT_DIR"] = str(Path(args.output_dir))
    stdout = run([str(BINARY)], env=env)
    if not args.formal:
        if "CHINA_US_RELAY_001_SMOKE_OK" not in stdout:
    ...
    print("CHINA_US_RELAY_001_FORMAL_COMPLETE")
    return 0
