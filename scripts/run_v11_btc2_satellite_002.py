...
TRIAL_ID = "ATM-SVP3-V11-BTC2-SATELLITE-002"
CANDIDATE_ID = "S-V11-BTC2-SATELLITE-002"
FRAGMENT = ROOT / "tools/v11_btc2_satellite_002.swiftpart"
LOGIC = ROOT / "tools/v11_btc2_satellite_001_logic.swift"
ASSEMBLED = Path("/private/tmp/atm_v11_btc2_satellite_002.swift")
BINARY = Path("/private/tmp/atm_v11_btc2_satellite_002")
...
def validate_document(document: dict) -> None:
    ...
    if document.get("metric_method") != "observed_periods_per_calendar_year":
        raise RuntimeError("mixed-calendar metric method drifted")
    for key in ("candidate", "v11_matched_control"):
        if document[key].get("metric_method") != "observed_periods_per_calendar_year":
            raise RuntimeError(f"{key} metric method drifted")
        periods = float(document[key].get("periods_per_year", 0))
        if not (360.0 <= periods <= 370.0):
            raise RuntimeError(f"{key} periods/year not mixed-calendar daily: {periods}")
...
def parse_formal_stdout(stdout: str) -> dict:
    prefix = "V11_BTC2_SATELLITE_002_FORMAL_JSON="
    lines = [line[len(prefix):] for line in stdout.splitlines() if line.startswith(prefix)]
    if len(lines) != 1 or "V11_BTC2_SATELLITE_002_FORMAL_OK" not in stdout:
...
def main() -> int:
    ...
    compile_binary()
    env = os.environ.copy()
    env.update({"ATM_HISTORY_FIXTURE": args.fixture, "ATM_V11_BTC2_SATELLITE_002": "1"})
    if args.formal:
        env["ATM_V11_BTC2_SATELLITE_002_FORMAL"] = "1"
        env["ATM_V11_BTC2_SATELLITE_002_OUTPUT_DIR"] = args.output_dir
    stdout = run([str(BINARY)], env=env)
    if not args.formal:
        if "V11_BTC2_SATELLITE_002_SMOKE_OK" not in stdout:
            print(stdout, end="")
    ...
    print("V11_BTC2_SATELLITE_002_FORMAL_COMPLETE")
    return 0
