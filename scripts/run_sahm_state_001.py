...
ROOT=Path(__file__).resolve().parents[1]; TRIAL_ID='ATM-SVP3-SAHM-STATE-001'; CANDIDATE_ID='S-SAHM-STATE-001'; FRAGMENT=ROOT/'tools/sahm_state_001.swiftpart'; LOGIC=ROOT/'tools/sahm_state_001_logic.swift'; ASSEMBLED=Path('/private/tmp/atm_sahm_state_001.swift'); BINARY=Path('/private/tmp/atm_sahm_state_001'); SAHM=ROOT/'tools/research-results/strategy-validation/external-data/ATM-SVP3-SAHM-STATE-001/SAHMREALTIME.csv'
...
def parse(stdout):
 prefix='SAHM_STATE_001_FORMAL_JSON='; rows=[x[len(prefix):] for x in stdout.splitlines() if x.startswith(prefix)]
 if len(rows)!=1 or 'SAHM_STATE_001_FORMAL_OK' not in stdout: raise RuntimeError('formal output incomplete')
 d=json.loads(rows[0]); assert d['trial_id']==TRIAL_ID and d['candidate_id']==CANDIDATE_ID and abs(d['sahm_trigger']-.50)<1e-12 and d['review_day_floor']==15 and d['prior_month_only'] is True and d['evaluation_start']=='2002-07-15' and abs(d['fee_percent_per_trade']-1)<1e-12 and abs(d['slippage_percent_per_trade']-.05)<1e-12; return d
def evaluate(d):
...
def main():
 ap=argparse.ArgumentParser(); ap.add_argument('--fixture',required=True); ap.add_argument('--output-dir'); ap.add_argument('--formal',action='store_true'); a=ap.parse_args(); compile_binary(); env=os.environ.copy(); env.update({'ATM_HISTORY_FIXTURE':a.fixture,'ATM_SAHM_STATE_001':'1','ATM_SAHM_STATE_001_SERIES':str(SAHM)}); 
 if a.formal: env.update({'ATM_SAHM_STATE_001_FORMAL':'1','ATM_SAHM_STATE_001_OUTPUT_DIR':a.output_dir})
 stdout=run([str(BINARY)],env=env)
 if not a.formal:
  if 'SAHM_STATE_001_SMOKE_OK' not in stdout: print(stdout,end=''); raise RuntimeError('smoke failed')
  print(stdout,end=''); return 0
 d=parse(stdout); e=evaluate(d); write_outputs(Path(a.output_dir),d,e); print(json.dumps({'trial_id':TRIAL_ID,'candidate_id':CANDIDATE_ID,**e},ensure_ascii=False,sort_keys=True)); print('SAHM_STATE_001_FORMAL_COMPLETE'); return 0
