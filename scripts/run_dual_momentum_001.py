...
ROOT=Path(__file__).resolve().parents[1]; TRIAL_ID='ATM-SVP3-DUAL-MOMENTUM-001'; CANDIDATE_ID='S-DUAL-MOMENTUM-001'; FRAGMENT=ROOT/'tools/dual_momentum_001.swiftpart'; LOGIC=ROOT/'tools/dual_momentum_001_logic.swift'; ASSEMBLED=Path('/private/tmp/atm_dual_momentum_001.swift'); BINARY=Path('/private/tmp/atm_dual_momentum_001')
...
def parse(stdout):
 prefix='DUAL_MOMENTUM_001_FORMAL_JSON='; rows=[x[len(prefix):] for x in stdout.splitlines() if x.startswith(prefix)]
 if len(rows)!=1 or 'DUAL_MOMENTUM_001_FORMAL_OK' not in stdout: raise RuntimeError('formal output incomplete')
 d=json.loads(rows[0]); assert d['trial_id']==TRIAL_ID and d['candidate_id']==CANDIDATE_ID and d['lookback_sessions']==252 and d['evaluation_start']=='2002-07-03' and abs(d['fee_percent_per_trade']-1)<1e-12 and abs(d['slippage_percent_per_trade']-.05)<1e-12; return d
def evaluate(d):
...
def main():
 ap=argparse.ArgumentParser(); ap.add_argument('--fixture',required=True); ap.add_argument('--output-dir'); ap.add_argument('--formal',action='store_true'); a=ap.parse_args(); compile_binary(); env=os.environ.copy(); env.update({'ATM_HISTORY_FIXTURE':a.fixture,'ATM_DUAL_MOMENTUM_001':'1'}); 
 if a.formal: env.update({'ATM_DUAL_MOMENTUM_001_FORMAL':'1','ATM_DUAL_MOMENTUM_001_OUTPUT_DIR':a.output_dir})
 stdout=run([str(BINARY)],env=env)
 if not a.formal:
  if 'DUAL_MOMENTUM_001_SMOKE_OK' not in stdout: print(stdout,end=''); raise RuntimeError('smoke failed')
  print(stdout,end=''); return 0
 d=parse(stdout); e=evaluate(d); write_outputs(Path(a.output_dir),d,e); print(json.dumps({'trial_id':TRIAL_ID,'candidate_id':CANDIDATE_ID,**e},ensure_ascii=False,sort_keys=True)); print('DUAL_MOMENTUM_001_FORMAL_COMPLETE'); return 0
