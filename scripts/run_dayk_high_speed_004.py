#!/usr/bin/env python3
"""Run the preregistered 日K高速004 trial through the shared Swift research engine."""
...
TRIAL_ID = "ATM-SVP2-DAYK-HS-004"
CANDIDATE_ID = "S-DAYK-HIGH-SPEED-004"
FRAGMENT = ROOT / "tools/dayk_high_speed_004.swiftpart"
LOGIC = ROOT / "tools/dayk_high_speed_004_logic.swift"
ASSEMBLED = Path("/private/tmp/atm_dayk_high_speed_004.swift")
BINARY = Path("/private/tmp/atm_dayk_high_speed_004")
...
def validate_document(document: dict) -> None:
    ...
    expected_candidates = {"DKHS004-GOLD", "DKHS004-NASDAQ", "DKHS004-SP500", "DKHS004-COMBINED"}
...
def evaluate(document: dict) -> dict:
    combined = path_by_id(document, "candidate_paths", "DKHS004-COMBINED")
    ...
    standalone = [
        path_by_id(document, "candidate_paths", "DKHS004-GOLD"),
        path_by_id(document, "candidate_paths", "DKHS004-NASDAQ"),
        path_by_id(document, "candidate_paths", "DKHS004-SP500"),
    ]
...
def parse_formal_stdout(stdout: str) -> dict:
    prefix = "DAYK_HIGH_SPEED_004_FORMAL_JSON="
    lines = [line[len(prefix):] for line in stdout.splitlines() if line.startswith(prefix)]
    if len(lines) != 1 or "DAYK_HIGH_SPEED_004_FORMAL_OK" not in stdout:
...
def main() -> int:
    ...
    env.update({
        ...
        "ATM_DAYK_HIGH_SPEED_004": "1",
    })
    if args.formal:
        env["ATM_DAYK_HIGH_SPEED_004_FORMAL"] = "1"
        env["ATM_DAYK_HIGH_SPEED_004_OUTPUT_DIR"] = str(Path(args.output_dir))
    stdout = run([str(BINARY)], env=env)
    if not args.formal:
        if "DAYK_HIGH_SPEED_004_SMOKE_OK" not in stdout:
    ...
    print("DAYK_HIGH_SPEED_004_FORMAL_COMPLETE")
    return 0
