#!/usr/bin/env python3
"""Run QUAL versus SPY matched-total-return production-path role trial without comparator mismatch."""
...
FRAGMENT = ROOT / "tools/qual_spy_total_return_matched_v1.swiftpart"
ASSEMBLED = Path("/private/tmp/atm_qual_spy_total_return_matched_v1.swift")
BINARY = Path("/private/tmp/atm_qual_spy_total_return_matched_v1")
...
def write_outputs(output_dir: Path, rows: dict[str, dict]) -> dict:
    ...
    document = {
        ...
        "strategy_lineage": "qual-vs-spy-total-return-production-role-v1",
...
def main() -> int:
    ...
    parser.add_argument("--qual-input", required=True)
    parser.add_argument("--spy-input", required=True)
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--formal", action="store_true")
    args = parser.parse_args()
    for path in (Path(args.fixture), Path(args.qual_input), Path(args.spy_input)):
        if not path.is_file():
    ...
    env.update(
        {
            ...
            "ATM_QUAL_SPY_TR_MATCHED_V1": "1",
            "ATM_QUAL_SPY_TR_MATCHED_V1_QUAL_PATH": args.qual_input,
            "ATM_QUAL_SPY_TR_MATCHED_V1_SPY_PATH": args.spy_input,
        }
    )
    output = Path(args.output_dir)
    if args.formal:
        env["ATM_QUAL_SPY_TR_MATCHED_V1_FORMAL"] = "1"
        env["ATM_QUAL_SPY_TR_MATCHED_V1_OUTPUT_DIR"] = str(output)
    stdout = run([str(BINARY)], env=env, timeout=300)
    if "QUAL_SPY_TR_MATCHED_V1_COMPLETE" not in stdout:
    ...
    if not args.formal:
        ...
        print("QUAL_SPY_TR_MATCHED_V1_SMOKE_OK")
        return 0
    ...
    print("QUAL_SPY_TR_MATCHED_V1_FORMAL_COMPLETE")
    return 0
