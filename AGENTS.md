...
- Factor-library archival hard rule: every formal factor candidate that reaches a recorded RESULT must be exported and published to the FlyingrtxFast factor library without waiting for a user reminder, regardless of PASS/FAIL. Preserve the truthful lifecycle: robust-admitted factors may advance only under protocol/library transition rules; failed and near-miss factors must be archived as rejected; prospective shadows remain explicit research/shadow entries. A factor trial is not operationally closed until the production import is completed with failed_count=0, and the research ledger must be reconciled against the library so no completed factor is omitted. Strategy-architecture candidates such as HR-A/B/C are not factors and are excluded.
