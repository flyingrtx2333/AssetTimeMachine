...
- Formal strategy research began under frozen `ATM-SVP-1`; the current governance protocol is `docs/strategies/validation/strategy-validation-protocol-v2.md` (`ATM-SVP-2`). V2 inherits V1 and changes only the G4 role-preserving generalization comparison frame to a same-common-window identity reference. It does **not** change V11 targets/parameters and does **not** restart the prospective OOS clock. Before executing any formal candidate family, parameter grid, factor screen, ablation, pristine holdout, or execution-stress test, append a schema-valid `PREREGISTER` record to `tools/research-results/strategy-validation/trial-ledger.jsonl` and commit the preregistration, dataset/holdout manifest, and executable Swift research code before looking at the formal result.
- ATM-SVP-1 remains independently frozen by `protocol-freeze-v1.json`; ATM-SVP-2 is independently frozen by `protocol-freeze-v2.json`. Never edit either frozen protocol document/policy in place. A further governance change requires ATM-SVP-3. Validate the selected manifest, its matching policy/freeze SHA, the ledger, gate bookkeeping, and recorded result artifacts with `python3 scripts/validate_strategy_protocol.py --manifest <protocol-manifest>`.
- Formal runs must pass `scripts/strategy_validation_run_guard.py`; prefer `scripts/strategy_validation_formal_run.py` so the exact command, Git commit, stdout/stderr, and run-authorization receipt are captured. Formal RESULT records must bind to the preregistration record hash and cover dataset/result evidence with SHA-256 manifests generated by `scripts/strategy_validation_artifact_manifest.py`.
- Never shrink the historical trial count to manufacture a retrospective DSR for V11; its pre-protocol total trial count is incomplete and must remain `NOT_CERTIFIED_LEGACY_TRIAL_COUNT` unless a genuinely complete census is reconstructed. For post-protocol candidate families, DSR is allowed only with the complete trial family and the explicit completeness certification flag.
- G3 / model-selection hard rule: **every formal research attempt counts**, including failed candidates, discarded parameter points, ablations, factor screens, alternative data transformations, and follow-up variants. Never report only the selected winner. Before execution, preregister the full candidate family/search budget and selection rule; after execution, preserve and report every candidate result. If the complete candidate family or historical trial count is unknown, G3 must stay `PARTIAL`/`NOT_CERTIFIED` and must never be relabeled PASS from PBO, parameter smoothness, or one favorable subset alone.
- Multiple-testing hard rule: do not open a new parameter grid, factor family, asset set, data transformation, or follow-up search merely because the previous formal result was weak. Any permitted follow-up must be explicitly allowed by the preregistration; otherwise it is a new research lineage/trial family and must be preregistered before results are viewed. Never keep searching until a Sharpe/DSR/PBO threshold passes.
- Evidence-retention hard rule: failures, `INCONCLUSIVE`, `INVALID`, and rejected candidates are durable evidence and must remain in the ledger/artifact history. Do not delete, overwrite, hide, or exclude them from trial-count / DSR / PBO accounting just because they are not product candidates.
...
- Latest TestFlight release: version `1.14` build `205`, Delivery UUID `842b2a4d-c40d-4b0d-beda-5c544ee6a6dc`, App Store Connect `BUILD-STATUS: VALID` and internal `IN_BETA_TESTING`, artifact directory `build/TestFlight-1.14-205`. The archive was built from clean, pushed source commit `bdd0c5b`.
- Build 205 restores the complete Time Machine changes: two-decimal CNY for selected total/net assets and liabilities, a gold-equivalent gram row when gold is enabled, green positive surplus bars, exact daily net assets and change from start, and tap-to-dismiss chart selection. Simulator Debug build and three-locale audit passed before archive.
- Earlier TestFlight builds: `204` (Delivery UUID `75a089d6-ae0e-46e9-954b-c512582f113b`), `203` (Delivery UUID `884d8f27-6d14-4a23-87e8-4483df6a86ff`), `202` (Delivery UUID `7b63d6d5-0e90-4ae4-bb3e-6c43624932b3`), and `201` (Delivery UUID `b2283a01-2be6-42ff-9eea-280bced94ed2`). The prior note incorrectly attributed build 204's UUID to build 202.
- Before future TestFlight releases, check the highest build in App Store Connect, verify every intended UI change is committed and pushed in the checkout used for archiving, and record that exact source commit with the Delivery UUID. Multiple local checkouts exist and previously diverged.

- Earlier TestFlight release: version `1.14` build `201`, Delivery UUID `b2283a01-2be6-42ff-9eea-280bced94ed2`, App Store Connect status `BUILD-STATUS: VALID`, artifact directory `build/TestFlight-1.14-201`.
