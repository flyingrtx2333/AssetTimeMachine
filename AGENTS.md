...
- Factor-library promotion hard rule: when a newly researched factor reaches the preregistered robust admission standard (`ROBUST_FACTOR_PASS=true` or the exact successor gate defined by a future protocol), immediately export and publish it to the FlyingrtxFast factor library using the existing manifest validate/import/complete pipeline, then read the remote factor record back to verify the lifecycle/evidence round trip. Do not wait for a separate user reminder. Failed or near-miss factors remain durable research evidence but must not be mislabeled as validated/promoted factors; prospective shadows may be published only with their explicit `research`/shadow lifecycle status.
