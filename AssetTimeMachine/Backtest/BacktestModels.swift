...
enum AdvancedBacktestStrategyMode: String, Codable {
    ...
    var detail: String {
        ...
        case .riskContributionCashConfidenceRouter:
            return AppLocalization.string("当前无融资夏普冠军：保留15%快速桥接、20.75%水下恢复袖套和原有现金约束，并新增在线领导权兑现率校准。每次黄金、纳指、标普或A股之间发生主导权交接后，使用严格T−1数据观察后续10个交易日，记录新领导者是否跑赢旧领导者；以Beta(2,2)先验持续更新各类新领导者的历史兑现率。未来再次切换到该领导者时，最低执行50%，历史兑现越稳定越接近完整迁移；降风险和转现金不受延迟。总仓位封顶100%，现金不得为负，沿用20%目标偏差和底层关键切换调仓，统一计入1%交易费与0.05%滑点。")
        case .strongVolControlledRotation:
