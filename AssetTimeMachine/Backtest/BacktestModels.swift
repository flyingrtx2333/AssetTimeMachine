...
enum AdvancedBacktestStrategyMode: String, Codable {
    ...
    var detail: String {
        ...
        case .riskContributionCashConfidenceRouter:
            return AppLocalization.string("当前无融资夏普冠军：以16.5%快速桥接和22%水下恢复袖套的双引擎路由为底层，继续使用基础仓位置信度、四市场宽度、2.5%至5.1%回撤谷区和低波动三市场扩散。执行层新增三项现金约束剪枝：总仓位20%至50%且A股主导时清除低置信度A股残余；总仓位至少70%、纳指主导且站上MA40时，纳指按82%、其他风险资产按78%执行；总仓位不超过20%且纳指主导时清除纳指残余。总仓位封顶100%，现金不得为负；目标差异超过20%或底层关键切换时调仓。全部信号严格使用T−1数据，并统一计入1%交易费与0.05%滑点。")
        case .strongVolControlledRotation:
