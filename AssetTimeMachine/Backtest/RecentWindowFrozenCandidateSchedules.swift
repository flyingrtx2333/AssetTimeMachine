    ...
    static func load(mode: RecentWindowOverlayStrategy.Mode, executionDates: [Date]) -> FrozenTargetSchedule? {
        ...
        case .volatilityManagedIdleCash:
            reason = AppLocalization.format(
                "T-1 63日组合波动 %@，仓位倍数 %@",
                signal("realized_volatility") ?? AppLocalization.string("尚不足"),
                signal("multiplier") ?? "1"
            )
        case .pairSpreadZ252Shift25:
            reason = AppLocalization.format(
                "T-1 配对Z分数：ONEQ/SPY %@，510300/510210 %@；阈值±1",
                signal("us_oneq_spy_z252") ?? AppLocalization.string("尚不足"),
                signal("china_510300_510210_z252") ?? AppLocalization.string("尚不足")
            )
        case .goldEquityRelativeZ252Shift25:
            reason = AppLocalization.format(
                "T-1 黄金/权益Z分数 %@，黄金转移比例 %@",
                signal("z_score") ?? AppLocalization.string("尚不足"),
