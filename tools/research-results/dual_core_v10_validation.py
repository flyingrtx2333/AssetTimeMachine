...
FILES={
    'DualCoreV1':Path('/private/tmp/DualCoreV1_current.csv'),
...
def main():
    loaded={n:load(p) for n,p in FILES.items()};dates=loaded['DualCoreV1'][0]
    if any(d!=dates for d,_ in loaded.values()):raise RuntimeError('date mismatch')
    full={};folds={};boots={}
    print('DUAL_CORE_V10_VALIDATION')
    ...
    base=full['DualCoreV1'];m=full['Unified122V10'];bt=boots['Unified122V10'];bf=folds['DualCoreV1'];cf=folds['Unified122V10']
    ...
    ok=(m.cagr>=.14 and m.sharpe>=1.50 and m.mdd<=.085 and abs(m.sharpe-base.sharpe)<=.020 and gt1>=5 and worst>0 and bt[0]>=1.15 and bt[3]<=.15 and (beat_sh>=4 or better_dd>=6))
    print(f'GATES,Unified122V10,{"PASS" if ok else "FAIL"},folds_sharpe_gt1={gt1}/7,worst_fold={worst:.6f},fold_sharpe_ge_v1={beat_sh}/7,fold_mdd_le_v1={better_dd}/7,sharpe_delta={m.sharpe-base.sharpe:.6f}')
