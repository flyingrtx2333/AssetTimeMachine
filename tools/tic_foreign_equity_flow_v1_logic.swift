...
nonisolated enum TICForeignEquityFlowV1Logic {
    struct Point: Equatable {
        let reportMonth: String
        let availableDate: String
        let netForeignPurchasesMillions: Double
        let regime: String
    }
    ...
    static func latestRiskOn(
        ...
        var latest: Point?
        for point in points {
            guard point.netForeignPurchasesMillions.isFinite else { continue }
        ...
        return point.netForeignPurchasesMillions < 0
    }
    ...
    static func latestPoint(
        ...
        guard let signal = parsedDay(signalDate) else { return nil }
        guard let point = points.last(where: {
            $0.availableDate <= signalDate && $0.netForeignPurchasesMillions.isFinite
        }), let available = parsedDay(point.availableDate) else { return nil }
        let staleDays = calendar.dateComponents([.day], from: available, to: signal).day ?? Int.max
        guard staleDays >= 0, staleDays <= maxStaleDays else { return nil }
        return point
    }
