...
@main
private enum AssetAgnosticFinalHoldoutV3Main {
    static func main() throws {
        let args = Array(CommandLine.arguments.dropFirst())
        let fixturePath = value("--fixture", in: args)
            ?? "tools/fixtures/backtest-history/asset_agnostic_final_holdout_v3.json"
        let outputPath = value("--output", in: args)
            ?? "tools/research-results/asset-agnostic-final-holdout-v3.json"
        ...
        let singleFolds = symbols.compactMap { symbol -> FinalHoldoutFold? in
            guard let result = AssetAgnosticBacktestEngine.runSparseV3(history: history, symbols: [symbol], config: config) else { return nil }
            return .init(name: symbol, symbols: [symbol], strategy: result.performance, benchmark: result.benchmark)
        }
        let basket: FinalHoldoutFold?
        if let result = AssetAgnosticBacktestEngine.runSparseV3(history: history, symbols: symbols, config: config) {
            basket = .init(name: "final_cross_market_basket_v3", symbols: symbols, strategy: result.performance, benchmark: result.benchmark)
        } else {
            basket = nil
        }
        ...
        // These thresholds are preregistered in docs/strategies/asset-agnostic-generalization-v3.md.
        var failures: [String] = []
        ...
        let report = FinalHoldoutReport(
            ...
            strategyVersion: "asset-agnostic-sparse-frozen-v3",
            symbols: symbols,
        ...
        print("FINAL ASSET-OOS HOLDOUT — sparse frozen V3")
        print("Markets: \(symbols.joined(separator: ", "))")
        for fold in singleFolds { print(format(fold)) }
        ...
        print("FINAL_GENERALIZATION_V3: \(failures.isEmpty ? "PASS" : "FAIL")")
    ...
    private static func symbolRenameInvariant(
        ...
        let mapping = Dictionary(uniqueKeysWithValues: symbols.enumerated().map { index, symbol in
            (symbol, "blind_holdout_asset_\(symbols.count - index)")
        })
