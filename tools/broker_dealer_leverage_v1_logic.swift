...
nonisolated enum BrokerDealerLeverageV1Logic {
    struct Point: Equatable {
        let quarter: String
        let availableDate: String
        let annualLogLeverageGrowth: Double
    }
    ...
    static func latestFavorable(
        points: [Point],
        signalDate: String,
        maxStaleDays: Int = 150
    ) -> Bool? {
        guard maxStaleDays >= 0, let signal = parsedDay(signalDate) else { return nil }
        var latest: Point?
        for point in points {
            guard point.annualLogLeverageGrowth.isFinite else { continue }
        ...
        return point.annualLogLeverageGrowth < 0
    }
    ...
    static func retainedUSTarget(
        ...
        var target = currentBase.reduce(into: [String: Double]()) { result, item in
            ...
            if weight > 0, weight.isFinite { result[item.key] = weight }
        }
