...
@main
enum DayKHighSpeed004LogicTest {
    ...
    static func main() {
        var rising = (0..<210).map { index -> DayKHighSpeed004Bar in
        ...
        let signal = DayKHighSpeed004Logic.signal(bars: rising, executionIndex: 210, requireTrend: true)
        ...
        require(DayKHighSpeed004Logic.signal(bars: shallow, executionIndex: 210, requireTrend: true) == nil, "shallow pullback must not trigger")

        var falling = (0..<210).map { index -> DayKHighSpeed004Bar in
        ...
        require(DayKHighSpeed004Logic.signal(bars: falling, executionIndex: 210, requireTrend: true) == nil, "falling trend must be filtered")
        require(DayKHighSpeed004Logic.signal(bars: falling, executionIndex: 210, requireTrend: false) != nil, "matched pullback control must remove only trend gate")
        ...
        let changed = DayKHighSpeed004Logic.signal(bars: currentPathChanged, executionIndex: 210, requireTrend: true)
        ...
        print("DAYK_HIGH_SPEED_004_LOGIC_TEST_OK")
    }
}
